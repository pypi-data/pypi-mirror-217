...
class CiscoDeviceSupport(ChangeLoggedModel):
    ...
    # Property field for desired release
    @property
    def desired_release_status(self):
        """
        Compare the releases to set the status for the desired release.
        ...
        if all(isinstance(value, str) for value in [self.desired_release, self.recommended_release]):
            return True if self.desired_release in self.recommended_release else False
        return False
    ...
    # Property field for desired release
    @property
    def current_release_status(self):
        """
        Compare the releases to set the status for the current release.
        Verify if the desired release match the current release.
        """
        ...
        if all(isinstance(value, str) for value in [self.current_release, self.desired_release]):
            return True if self.current_release in self.desired_release else False
        return False
