...
class MetaLogistic(_MetaLogisticMonoFit):
	...
	def __init__(
		...
		if a_vector is not None:
			# We are done
			self.__dict__.update(_MetaLogisticMonoFit(a_vector=a_vector, lbound=lbound, ubound=ubound).__dict__.copy())
			self.fit_method_requested = None
			self.term_requested = None
			return
