        ...
        while self.running:
            if self.type == task.C_STATE.DOWNLOADING:
                ...
                if taskItem:
                    ...
                    url = taskItem["url"]
                    one_zip_switch = taskItem["one_zip_switch"]
                    utils.logInfo(f"==={curGroupId} begin", True)
                    utils.logInfo(f"==={curGroupId} GetTask: {taskItem}", True)
                    curDownloadDir, allCount = downloader.download(url, curGroupId, one_zip_switch)
            ...
            if self.type == task.C_STATE.TEMPLATEING and task.canTemplate(lock):
                ...
                if taskItem:
                    ...
                    if taskItem["video_merge_num"] > 0 and taskItem["video_merge_second"] > 0 and one_zip_switch == False:
                        # random cutter
    ...
    try:
        ...
        if len(res.content) > 0:
            ...
            if len(data) > 0 and "id" in data:
                ...
                tag = data["tag"]
                oneZipSwitch = int(data["one_zip_switch"])
                verticalScreen = False
                if "VerticalScreen=true" in url:
                    verticalScreen = True
                    url = url.replace("VerticalScreen=true","")
                return curGroupId, url, template_name_list, video_merge_num, video_merge_second, img_to_video, split_video, add_text, verticalScreen, tag, oneZipSwitch
    except Exception as e:
         utils.logInfo(e)
    ...
    while (os.path.exists(os.path.join(thisFileDir, "stop.now")) == False):
        if task.canAddDownload(lock):
            try:
                ...
                #     "one_zip_switch": True
                # })
                curGroupId, url, template_name_list, video_merge_num, video_merge_second, img_to_video, split_video, add_text, verticalScreen, tag, oneZipSwitch = getTask()
                if curGroupId:    
                    task.push(lock, {
