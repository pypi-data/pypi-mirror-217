...
def ftpdownload(url, curGroupId, one_zip_switch):
    global downloadAllCount
        ...
        if file_name.endswith('.zip'):
            try:
                with zipfile.ZipFile(os.path.join(local_dir, file_name), 'r') as zip_ref:
                    extractPath = local_dir
                    if one_zip_switch:
                        fname = file_name[0:file_name.index(".")]
                        extractPath = os.path.join(local_dir, fname)
                        if os.path.exists(extractPath) == False:
                            os.makedirs(extractPath)
                    zip_ref.extractall(extractPath)
                downloadAllCount += len(os.listdir(extractPath))
            except Exception as e:
                downloadAllCount += 0
            finally:
                os.remove(os.path.join(local_dir, file_name))
...
def download(url, curGroupId, one_zip_switch):
    global downloadAllCount
    downloadAllCount = 0
    ...
    while downloadAllCount == 0 and retryDownload != 0:
        if "http" in url or "https" in url:
            ...
            downloadDir = curDownloadDir(curGroupId)
            ftpdownload(url, curGroupId, one_zip_switch)
        retryDownload -= 1
