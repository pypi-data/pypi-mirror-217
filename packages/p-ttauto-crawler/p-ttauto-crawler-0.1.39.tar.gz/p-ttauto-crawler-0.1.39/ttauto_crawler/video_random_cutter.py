...
def real_video_cutter(input_dir, video_merge_second, video_merge_num, output_dir):
    interval_indexs = {}
    intervals = {}
    unavailable_videos = []
...
def video_cutter(input_dir, curGroupId, params):
    ...
    if one_zip_switch:
        dirs = [os.path.join(root, dir) for root, dirs, files in os.walk(input_dir)
                for dir in dirs]
        for d in dirs:
            utils.logInfo(f"      ==={curGroupId} subdir {d} video_cutter to video", True)
            real_video_cutter(d, video_merge_second, video_merge_num, output_dir)
            utils.logInfo(f"      ==={curGroupId} finish", True)
    else:
        real_video_cutter(input_dir, video_merge_second, video_merge_num, output_dir)
