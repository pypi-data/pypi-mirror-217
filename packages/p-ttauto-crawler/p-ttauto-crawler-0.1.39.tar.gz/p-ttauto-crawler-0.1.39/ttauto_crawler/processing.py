...
def processAllImage(curGroupId, data, curDownloadDir, tpname, addRandomText, img2videoCnt, tag, randomChoose, page=0):
    src = curDownloadDir
...
def realProcessToVideo(curDownloadDir,params,page=0):
    ...
    #imgs
    if img_to_video > 0:
        randomChoose = not one_zip_switch
        if len(crawler_template_name) > 0:
            for tpname in crawler_template_name:
                templateDir = os.path.join(genertor_binary.randomEffectPath(""), tpname)
                official_template_list.append(templateDir)
                utils.logInfo(f"      ==={curGroupId} {img_to_video} image to video with {tpname}", True)
                processAllImage(curGroupId, data, curDownloadDir, tpname, addRandomText, img_to_video, tag, randomChoose, page)
        else:
            utils.logInfo(f"      ==={curGroupId} {img_to_video} image to video", True)
            processAllImage(curGroupId, data, curDownloadDir, "", addRandomText, img_to_video, tag, randomChoose, page)
    #process template
...
def processToVideo(curDownloadDir, params):
    ...
    if one_zip_switch:
        ...
        idx = 0
        for d in dirs:
            utils.logInfo(f"      ==={curGroupId} subdir {d} process to video", True)
            realProcessToVideo(d, params, idx)
            idx+=1
            utils.logInfo(f"      ==={curGroupId} finish[{len(os.listdir(outputDir))}]", True)
    else:
        realProcessToVideo(curDownloadDir, params, 0)
