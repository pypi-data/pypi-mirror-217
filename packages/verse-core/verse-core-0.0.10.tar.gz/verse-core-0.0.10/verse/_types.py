...
class Comparison(Expression):
    ...
    def __str__(self) -> str:
        _str = f"{_serialize(self.lhs)} {self.op.value} "
        if self.op == ComparisonOp.BETWEEN:
            val = cast(list, self.rhs)
            _str = _str + f"{_serialize(val[0])} AND {_serialize(val[1])}"
        else:
            _str = _str + _serialize(self.rhs)
        return _str
...
class Update:
    def __init__(self, operations: List[UpdateOperation] = []):
...
class Output:
    def __init__(self, operations: Optional[List[OutputTerm]] = None):
    ...
    def add(self, source: ExpressionType, dest: ExpressionType):
        self.operations.append(OutputTerm(source=source, dest=dest))

    def add_field(self, field: str, dest: ExpressionType):
        self.operations.append(OutputTerm(source=Field(field), dest=dest))
...
class OutputTerm:
    def __init__(self, source: ExpressionType, dest: ExpressionType):
        self.source = source
        self.dest = dest

    def __str__(self) -> str:
        return f"{_serialize(self.source)} INTO {_serialize(self.dest)}"
