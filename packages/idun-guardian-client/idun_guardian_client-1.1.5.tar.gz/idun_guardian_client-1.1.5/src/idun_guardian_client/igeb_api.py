...
from .igeb_utils import retry, exit_system, stop_ongoing_recording
                ...
                async with websockets.connect(settings.WS_IDENTIFIER) as self.websocket:  # type: ignore
                    try:
                        if self.rec_started_time == None:
                            print(
                                "Sending to the cloud information that recording started"
                            )
                            check_recording_started()
                    except Exception as e:
                        log_unable_to_start(self.debug, e)
                        exit_system()
                    try:
