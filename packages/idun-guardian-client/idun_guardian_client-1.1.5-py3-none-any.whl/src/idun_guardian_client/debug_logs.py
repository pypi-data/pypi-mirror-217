...
def log_couldnot_stop_ongoing_rec(debug, rec):
    if debug:
        logging.warning(
            f"Could not stop recording {rec}, please try another time or stop them manually with the given code"
        )


def log_rec_ongoing_stopped_successfully(debug, rec):
    if debug:
        logging.info(f"Recording id {rec}, was successfully_stopped")


def log_able_to_start(debug):
    if debug:
        logging.info("You are now able to start a new recording")
...
def log_rec_stopped(debug):
    if debug:
        logging.info("The Recording has been correctly stopped")


def log_unable_to_start(debug, err):
    if debug:
        logging.info(
            f"We could not start the recording, we will retry to fix the problem. \n Error: {err}"
        )
...
def log_unable_to_stop(debug, err):
    if debug:
        logging.info(
            f"We could not stop the recording, we will retry to fix the problem. \n Error: {err}"
        )
...
def log_successfully_started(debug):
    if debug:
        logging.info("The Recording has been correctly initialized")
...
def log_not_able_to_connect_with_the_cloud(debug, string: str, status: str):
    if debug:
        logging.info(
            f"Not able to {string} the connection with the cloud , exiting with error number {status}, will re-try"
        )


def log_checking_recording(debug, string: str):
    if debug:
        logging.info(f"Cheking that the recording is {string}")


def log_not_client_found(debug):
    if debug:
        logging.info("Client not initialized")


def log_exception_in_disconnecting(debug):
    if debug:
        logging.info("Cannot disconnect, device already disconnected")
...
def log_exception_while_trying_connection(debug, err):
    if debug:
        logging.info("Exception raised while trying to connect:", err)


def log_timeout_while_trying_connection(debug):
    if debug:
        logging.info("Timeout for connection failure, will try to reconnect")
