...
def insert_and_advance_targets_from_domains(
    ...
    """
    targets = search_target(search_uid=search_uid)
    with db.connect() as con:
...
def search_target(search_uid: int, last_event_type: str = None):
    statement = """
        SELECT 
            e.id, 
            e.search_uid, 
...
def search_target_export(search_uid: int):
    statement = """
        SELECT 
            e.id, 
            e.search_uid, 
        ...
        FROM (
            ...
            FROM 
                event
            WHERE 
                type NOT IN ('comment','rating','generate','criteria', 'reject') 
                AND search_uid = :search_uid
            GROUP BY 
                domain, search_uid
        ) AS max_event
    ...
    with db.connect() as conn:
        ...
        targets = targets.merge(meta, left_on=["domain"], right_on=["domain"], how="left")
