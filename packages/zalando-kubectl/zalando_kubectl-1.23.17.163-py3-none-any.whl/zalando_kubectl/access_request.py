...
def _wait_for_approval(env, current_cluster, response, auth_header, timeout):
    ...
    approval_ui = response["result"]["_links"].get("approval_ui", {}).get("href")
    ...
    if approval_ui:
        click.echo(f"via the UI:   {approval_ui}")
    click.echo(f"via the CLI:  zkubectl cluster-access list --request-key {request_key} --okta\n")
