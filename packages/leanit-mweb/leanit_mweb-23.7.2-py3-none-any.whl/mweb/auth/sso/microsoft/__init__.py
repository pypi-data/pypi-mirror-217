    ...
    async def fetch_token(self, code: str, code_verifier: str) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.authority}/oauth2/v2.0/token",
                ...
                if resp.status >= 300:
                    logger.error(f"Error fetching token from microsoft: ({resp.status}) {await resp.text()} scope={','.join(scope)} client_id={self.client_id} redirect_uri={self.redirect_url}")
