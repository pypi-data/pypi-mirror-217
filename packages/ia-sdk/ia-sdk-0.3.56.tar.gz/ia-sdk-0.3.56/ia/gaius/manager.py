...
class TFInfo:
    ...
    def spawn(self) -> bool:
        ...
        if spawn_tf(container_name=self.container_name, api_key=self.api_key, port=self.port, docker_image=self.docker_image, env=self.environment, volumes=self.volumes):
            self.save_config_file()
