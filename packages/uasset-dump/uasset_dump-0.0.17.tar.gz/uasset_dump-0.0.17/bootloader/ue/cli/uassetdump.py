...
def run():
    ...
    # Generate the asset inventory as a JSON dump.
    from timeit import default_timer as timer
    start = timer()
    json_string = uasset_dump.dump_assets(arguments.asset_path)
    end = timer()
    logging.debug(f"Asset dump completed in {end - start}s")

    # Write the asset JSON dump into the specified file.
    logging.debug(f"Writing the asset JSON dump to {arguments.output_file}...")
