...
class Regression:
    ...
    def train_the_model(self):
        ...
        if self.cross_validation_k_folds > 1:
            ...
            # reverse scaling
            self.cross_validation_score = list(self.scaler.inverse_transform(np.asarray(self.cross_validation_score).reshape(-1, 1)).reshape(1, -1)[0])
    ...
    def RMSE_history(self):
        ...
        data = {
            'x':list(train_sizes),
            'y1':list(train_scores_mean),
            'y2':list(test_scores_mean),
            'title':title,
        }
        return data
