...
class MXSuperThing(MXBigThing):
    ...
    def _handle_MS_SCHEDULE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        if not target_super_service:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super Service {super_schedule_msg.super_service_name} is not exist...', 'red')
            return False
        ...
        if super_schedule_msg.topic_error or super_schedule_msg.payload_error:
            ...
            return False
        if not self._check_super_service_available(target_super_service):
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super Service {target_super_service.get_name()} is not available...', 'red')
            return False
    ...
    def _handle_MS_EXECUTE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        if not target_super_service:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super Service {super_execute_msg.super_service_name} is not exist...', 'red')
            return False
    ...
    def _handle_MS_RESULT_SCHEDULE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        if not target_super_service:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super Service {subschedule_result_msg.super_service_name} is not exist...', 'yellow')
            return False
    ...
    def _handle_MS_RESULT_EXECUTE(self, msg: mqtt.MQTTMessage) -> bool:
        ...
        if not target_super_service:
            MXLOG_DEBUG(f'[{get_current_function_name()}] Super Service {sub_service_execute_result_msg.super_service_name} is not exist...', 'yellow')
            return False
        ...
        try:
            ...
            for middleware in service_list.service_list:
                ...
                if not middleware_name:
                    MXLOG_DEBUG(f'[{get_current_function_name()}] Middleware name is not exist', 'red')
                    return False
    ...
    def _check_super_service_available(self, super_service: MXSuperFunction):
