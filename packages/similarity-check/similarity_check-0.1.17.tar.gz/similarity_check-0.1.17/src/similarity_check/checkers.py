...
class sentence_tranformer_checker():
    def __init__(
        ...
        if encode_target:  
            ...
            self.encoded_targets = {(k): (self.model.encode(v, batch_size=self.encode_batch, show_progress_bar=True,  normalize_embeddings=True)) for k, v in self.targets.items()} # encode the targets
    ...
    def match(
        ...
        if isinstance(source, pd.DataFrame):
            for source_col, _ in source_mapping.values():
                if source_col not in source.columns:
                    raise KeyError(f'the following source_col ({source_col}) not found in source DataFrame cloumns')

            self.source_df = source.reset_index(drop=True)
            sources = {source_col: [preprocess(sent, self.remove_punct, self.remove_stop_words, self.stemm, self.lang) for sent in source[source_col].tolist()] for source_col, _ in source_mapping.values()}
        else:
        ...
        if not hasattr(self, 'encoded_targets'):
            print('enocding targets: ')
            encoded_targets = {(k): (self.model.encode(v, batch_size=self.encode_batch, show_progress_bar=True,  normalize_embeddings=True)) for k, v in self.targets.items()} # encode the targets
        else:
        ...
        for i in tqdm(range(0, inputs_length, batch_size)):
            ...
            batch_top_cosine, batch_match_idxs = self.max_cosine_sim(encoded_inputs, encoded_targets, source_mapping , topn, threshold, batch_size, targets_length)
    ...
    def max_cosine_sim(self, encoded_inputs, encoded_targets, source_mapping, topn, threshold, inputs_length, targets_length):
        ...
        for combinition_target, (combinition_input, weight) in source_mapping.items():
            if len(encoded_inputs[combinition_input].shape) == 1:
                encoded_inputs[combinition_input] = encoded_inputs[combinition_input].unsqueeze(0)

            if len(encoded_targets[combinition_target].shape) == 1:
                encoded_targets[combinition_target] = encoded_targets[combinition_target].unsqueeze(0)
