...
def plantilla(
    proyecto=None, subproyecto=None, path_proyectos=None, new=False, globales=True
):
    ...
    try:
        ...
        for folder in site_packages:
            try:
                path_style = os.path.join(
                    folder, "plantilla", "styles", "mecon.mplstyle"
                )
        ...
        path_end_style = os.path.join(path_stylelib, "mecon.mplstyle")
        print(
            f'No se pudo importar el estilo. Mover a mano a el archivo "mecon.mplstyle" desde la carpeta de plantilla (ruta de python/Lib/site-packages/plantilla/styles) a {path_end_style} y activarlo con "plt.style.use("mecon.mplstyle")"'
        )
    ...
    if new:
        ...
        test_path = rf"C:/Users/Administrador/Documents/MECON/Proyectos/{proyecto}"
        ...
        if path_proyectos:
            ...
            )  # Get padre de la ruta del archivo
            path_proyecto = path + "/" + str(proyecto)
            path_data = os.path.dirname(os.path.dirname(path_proyecto)) + "/" + "data"
            if os.path.isdir(path_proyecto) == False:
                raise NameError(
                    f"El directorio {path_proyecto} no fue encontrado, revisar nombre del proyecto"
                )

        folders = []

        print(path_proyecto + rf"/scripts/{subproyecto}")
        print(path_data)

        if os.path.isdir(path_proyecto + rf"/scripts/{subproyecto}"):
            print("El directorio en scripts existe. Creando carpetas.")
            try:
                folders = [
                    r"/data_out",
                    rf"/data_out/{subproyecto}",
                    r"/docs",
                    r"/scripts",
                    rf"/scripts/{subproyecto}",
                    r"/outputs",
                    r"/outputs/figures",
                    rf"/outputs/figures/{subproyecto}",
                    r"/outputs/maps",
                    rf"/outputs/maps/{subproyecto}",
                    r"/outputs/tables",
                    rf"/outputs/tables/{subproyecto}",
                ]
            ...
            # Folders en /data/
            data_folders = [r"/data_raw", r"/data_proc"]
            ...
            raise NameError(
                f"El directorio {path_proyecto + f'//scripts//{subproyecto}'} no existe. Crear una carpeta en scripts con el nombre del subproyecto."
            )
        ...
        if globales:
            ...
            path_dataout = path_proyecto + rf"/data_out/{subproyecto}"
            path_scripts = path_proyecto + rf"/scripts/{subproyecto}"
            path_figures = path_proyecto + rf"/outputs/figures/{subproyecto}"
            path_maps = path_proyecto + rf"/outputs/maps/{subproyecto}"
            path_tables = path_proyecto + rf"/outputs/tables/{subproyecto}"
            path_programas = (
                os.path.dirname(os.path.dirname(os.path.dirname(path_proyecto)))
                + "/"
                + r"/0. Varios/scripts/Programas"
            )
        ...
        test_path = rf"C:/Users/Administrador/Documents/MECON/{proyecto}"
        ...
        if os.path.isdir(test_path):
            ...
            if os.path.isdir(path_user) == False:
                raise NameError(
                    "Error 404. Directorio no encontrado, revisar nombre del proyecto"
                )

        folders = []

        print(path_user + rf"/scripts/{subproyecto}")

        if os.path.isdir(path_user + rf"/scripts/{subproyecto}"):
            print("El directorio en scripts existe. Creando carpetas.")
            ...
            raise NameError(
                "El directorio en scripts no existe. Por favor, crear una carpeta en scripts con el nombre del subproyecto."
            )
        ...
        path_dataout = path_user + rf"/data/data_out/{subproyecto}"
        path_scripts = path_user + rf"/scripts/{subproyecto}"
        path_figures = path_user + rf"/outputs/figures/{subproyecto}"
        path_maps = path_user + rf"/outputs/maps/{subproyecto}"
        path_tables = path_user + rf"/outputs/tables/{subproyecto}"
        path_programas = (
            path_user
            + r'"C:/Users/Administrador/Documents/MECON/0. Varios/scripts/Programas"'
        )
...
def globales(proyecto, subproyecto=None, path_proyectos=None):
    ...
    path_tables = path_proyecto + rf"/outputs/tables/{subproyecto}"
    path_programas = (
        os.path.dirname(os.path.dirname(path_proyecto))
        + "/"
        + r"/0. Varios/scripts/Programas"
    )
