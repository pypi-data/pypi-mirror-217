...
def clone_codes_v2(code_refs: List[ResponseCodeRefsV2]):
    for code_ref in code_refs:
        mount_path = pathlib.PurePath(code_ref.mount_path)
        fallback = False
        ...
        if fallback:
            ...
            dirname = pathlib.PurePath(mount_path).parent
        ...
        if code_ref.git_ref:
            subprocess.run(["git", "switch", code_ref.git_ref, "--detach"], cwd=str(mount_path)).check_returncode()
