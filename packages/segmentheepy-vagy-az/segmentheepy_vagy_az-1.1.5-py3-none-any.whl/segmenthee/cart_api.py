...
def Score2Vote(score,threshold,eps) -> int:
...
def WillAbandonQ(state,modelpath) -> Union[None, Tuple[int, float]]:
    ...
    vote = Score2Vote(score, SCORE_THRESHOLD, SCORE_EPS)
    return (vote,score)

# Frustration point

def FrustrationReadyQ(state):
    return (is_browsing(state) and state["actioncount"] >= 5 and state["clickrate_var"] > 0.0)
