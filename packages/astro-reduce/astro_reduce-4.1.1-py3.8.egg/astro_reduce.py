...
from helpers import LDAC2ASC_TMP
...
@click.option('--scamp', is_flag=True,
              ...
              'after cosmetic reduction.')
@click.option('--astrometrynet', is_flag=True,
...
def cli(setup, clear, interpolate, verbose, tmppng, stkpng,
        sex, psfex, sexagain, scamp, astrometrynet, nomaster, nostack):
    ...
    if astrometrynet:
        listldac = glob('{}/*.ldac'.format(SEX_RES))
        for ldac in listldac:
            ldac2asc_cmd = LDAC2ASC_TMP.format(ldac,ldac.replace('.ldac','.dat'))
            if verbose:
                click.secho('  Submitting ldactoasc command: '
                            '{}'.format(ldac2asc_cmd), nl=True, fg='blue')
            system(ldac2asc_cmd)
        # make list of original files
        listfits = []
        for ffile in glob('{}/*.fits'.format(TMP)):
            if ffile in noastro:
                continue
            listfits.append(ffile)
        # make list of ldac files
        listdat = glob.glob('{}/*.dat'.format(SEX_RES))
