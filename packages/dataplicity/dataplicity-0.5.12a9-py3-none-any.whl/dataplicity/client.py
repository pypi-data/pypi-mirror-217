...
class Client(object):
    ...
    def load_features(self):
        ...
        log.warning("url? %s/features/", self.rpc_url)
        response = requests.get("%s/features/" % self.rpc_url, headers=headers)
        log.warning("RESPONSE? ", response.text)
        if response.ok:
            data = response.json()
    ...
    def migrate_to_device_token(self, serial, auth_token):
        ...
        """
        log.warning("Requesting device authentication token")
        json_data = {
            "serial": serial,
            "auth_token": auth_token
        }
        log.warning("SERIAL? %s // %s", serial, len(auth_token))
        response = requests.post(
            "%s/auth/" % self.rpc_url,
            json=json_data
        )
        log.warning("RESPON? %s", response.text)
        if response.ok:
