...
def compute_xy_coords(data, crs):
    ...
    if 'lonc' in data and 'latc' in data:
        data['xc'], data['yc'] = ddeq.misc.transform_coords(
            data.lonc, data.latc, input_crs=wgs84, output_crs=crs,
            use_xarray=True
        )
...
def compute_plume_coordinates(data, curve, show=False,
    ...
    if n_multiple_solutions > 0:
        pass #print('%d multiple solutions' % n_multiple_solutions)
...
def compute_plume_line_and_coords(data, crs, radius=25e3, pixel_size=2e3,
                                  do_coords=True, plume_area='area',
    ...
    crs :: cartopy.crs
        coordinate reference system used for computing local easting and northing

    radius :: float (in meters)
        an area around the detected plume that is included when fitting
        the center line (if `plume_area` == 'area') 
    ...
    reject_overlapping_sources (default: True)
        don't compute line and coords for plume detection overlapping several
        sources

    Returns
    +++++++
        data : input `data` with added variables
        curves : dict of center curves per source
    """
