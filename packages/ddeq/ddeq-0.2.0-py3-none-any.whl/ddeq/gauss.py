...
def Gaussian_plume_model(K, u, Q, xn, yn, BG=0.0, x0=0, y0=0, tau=14400, b=1,
    ...
    decay = ddeq.functions.decay_function(xn, x0, tau*60*60*u)
    ...
    plume = np.full(xn.shape, np.float(BG))
    plume[down] += c
...
def generate_params(curves, params, source, num, wind_speed,
    ...
    params.add(f'tau_{num}', tau_prior, min=1, max=200, vary=False) # Assume 4 hour decay time...
...
def gaussian_plume_estimates(this, overlapping_sources, curves,
                             priors, wind_speed, trace_gases=['NO2', 'CO2'],
    ...
    for num_gas, (gas, fit_decay) in enumerate(zip(trace_gases, fit_decay_times)):
        ...
        # Fit the data
        observations = this[f'{gas}_minus_estimated_background_mass'].values
        ...
        noise_level  = this[f'{gas}_minus_estimated_background_mass'].attrs.get('noise_level', np.nan)
...
def estimate_emissions(data, winds, sources, curves, gases, priors=None,
    ...
    Parameters
    ++++++++++
    fit_decay_times: boolean or list of booleans
