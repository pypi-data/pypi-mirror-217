...
def visualize(data, values, do_zoom=True, winds=None, show_clouds=True,
    ...
    if do_zoom and ('detected_plume' in data or zoom_on is not None):
        if 'detected_plume' in data:
            ...
            dy = 0.5 # FIXME: de-hardcode
            dx = 0.5

            xmin, xmax = np.nanmin(rlon_0) - dx, np.nanmax(rlon_0) + dx
            ymin, ymax = np.nanmin(rlat_0) - dy, np.nanmax(rlat_0) + dy
    ...
    if draw_gridlines:
        add_gridlines(ax, dlon=0.2, dlat=0.2)
...
def show_detected_plumes(data, curves, values, ld=None, add_polygons=True,
                         ...
                         add_origin=False, marker='+', label=None, do_zoom=True,
    ...
    fig = visualize(
        ...
        winds=winds, domain=domain, marker=marker, label=label, draw_gridlines=True
    )
        ...
        if source in curves and curves[source] is not None:
            ...
            if add_polygons:
                ...
                add_area(ax, d, curve, xa[:1], xb[:1], ya[:1], yb[:1],
                              ...
                              do_middle_lines=True, color=polygon_color)
...
def add_text(gases, data, result, title=None):
    ...
    text.append('\nWind:')
    text.append(f'• Mean wind: {result.wind_speed:.1f} m s$^{{-1}}$, '
                f'{result.wind_direction:.0f}°')

    angle = result['angle_between_curve_and_wind']
...
def plot_along_plume(axes, gases, ld, time, source, figwidth=6.97,
                     no2_scaling=1.0):

    if axes is None:
        fig, axes = plt.subplots(2,1, sharex=True, figsize=(6.97,4))
    else:
        fig = axes[0].get_figure()

    # TODO: use Mt for CO2 and kt for NOx?!?
    ...
    for ax, gas in zip(axes, gases):
        ...
        y = np.interp(x, ld['along'], factor * ld[f'{gas}_flux_fit'])

        label = 'Estimated emissions'
        ax.plot(x / 1e3, y, label=label, color='C0', ls='--')

        # labels
        for l in '1234Xx':
            gas = gas.replace(l, f'$_{l}$')

        ax.set_ylabel(f'{sub_numbers(gas)} flux [a.u.]')

    axes[-1].set_xlabel('Along-plume distance [km]')
...
def plot_gauss_result(data, results, sources, gas, curves, domain=None,
    ...
    ax2.set_title(f'Model (Q = {float(Q):.3g} kg/s, $\\tau$ = {float(tau):.1f} h)')
...
def plot_csf_result(gases, data, winds, line_densities, curves,
                    source, no2_scaling=1.0, domain=None, crs=None,
                    scalings=None, vmins=None, vmaxs=None):
    """
    Make plot showing result of cross-sectional flux method for one or two
    gases.
    """
    time = pandas.Timestamp(data.time.values)
    ...
    with plt.style.context({'font.size': 10}):
        ...
        for j, (i, gas) in enumerate(zip([1,0], gases)):
            ...
            vmin = None if vmins is None else vmins[j]
            vmax = None if vmaxs is None else vmaxs[j]
            scaling = 1.0 if scalings is None else scalings[j]

            fig = show_detected_plumes(data, curves, scaling * data[gas],
                                       gas=gas,
                                       ld=line_densities,
                                       add_polygons=True,
                                       winds=winds,
        ...
        if len(gases) == 1:
            ...
            raise ValueError('Plotting works only with one or two gases.')

        plot_along_plume(axes, gases, line_densities[source], time=time,
                         source=source, no2_scaling=no2_scaling)

        if len(gases) == 2: # FIXME
        ...
        if len(gases) == 2:
            ...
            plt.figtext(0.01, 0.47, text, va='top', linespacing=1.1)

        if line_densities[source].pixels.size == 0:
            return fig
        ...
        for j, (i, ax) in enumerate(zip(alongs, axes)):
...
def plot_lcsf_results(source_name, results, data, sources, gases,
                     true_emis=None, domain=None):
    '''\
    Function to plot the results provided by the light cross-sectional method
    for a given source.

    # INPUTS
    plot_source: Name of the source whose results are shown
    res_xr: Result xarray provided by the function ddeq.lcs.estimate_emissions
    l2_data: l2 data xarray
    gases: str or list of strings with trace gases
    true_emis: if not None should be a dictionary whose keys are equal to
               ['CO2', 'NO2']
    '''
    ...
    for i, gas in enumerate(gases):
        ...
        hh_arr.append(hh)
        leg_arr.append('Along-plume estimates')
        ...
        hh = ax.plot([xmin,100], 2 * [np.median(emis_estim)], lw=2,
                     color=colors[0])[0]
        hh_arr.append(hh)
        leg_arr.append('Median estimates')
        ...
        if true_emis is not None:
            hh = ax.plot([xmin,xmax], 2 * [true_emis[gas]], lw=2,
                         color='k')
            hh_arr.append(hh)
            leg_arr.append('True emissions')
            ymax = max([ymax, true_emis[gas]])

        #Legend
        ll = ax.legend(hh_arr, leg_arr, prop=dict(size=fs_leg), framealpha=0.5,
...
def plot_divergence_maps(NOx_div, CO2_div, lon_lat_center, lon_km, lat_km, grid_reso,
                         sources=None, ms=5, fontsize='x-large', cbar_fraction=0.051):
    longrid, latgrid, _, _ = ddeq.misc.grids(lon_lat_center, lon_km+grid_reso/2, lat_km+grid_reso/2, grid_reso)
    request = cimgt.OSM()
    ...
    c1 = ax1.pcolormesh(longrid, latgrid, NOx_div*1000,
                        shading='auto',
                        cmap=plt.cm.jet,
                        transform=ccrs.PlateCarree(),
                        alpha=0.3,
    ...
    c2 = ax2.pcolormesh(longrid, latgrid, CO2_div,
                        shading='auto',
                        cmap=plt.cm.jet,
                        transform=ccrs.PlateCarree(),
                        alpha=0.3,
