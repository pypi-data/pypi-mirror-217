...
def gridded_divergence(datasets, wind, date, latgrid, longrid, latgrid_km, longrid_km,
    ...
    for i, file in enumerate(datasets.read_date(date)):
        ...
        if species == 'CO2':
            ...
            data = ucat.convert_columns(data, 'ppmv', 'm-2', molar_mass='CO2',
                                        p=psurf)

        elif species == 'NO2':
            # Change NO2 units from molec/cm^2 to molec/m^2
            data = ucat.convert_columns(data, 'cm-2', 'm-2')
...
def estimate_emissions(datasets, wind_folder, sources,
                       ...
                       trace_gases=['CO2']):
    """
    Estimates the NOx and CO2 emissions for given sources over the chosen time
    series by computing the divergence map for each source and fitting a peak
    fitting model.
    """
    # Process start time
        ...
        for gas in trace_gases:
            ...
            for date in dates:
                ...
                wind = ddeq.wind.read_gnfra_wind(
                    wind_folder, date, latlims=latlims, lonlims=lonlims,
