...
def _mdl_caracteristic(model, config_sim: dict = None, filepath: str = None):
...
class _process_values_preliminary:
    ...
    def get_values(
        ...
        if type_mdl == 'Dense':
            ...
            X_test = X_test.reshape(X_test.shape[0], self.config_data['n_past'] * self.config_data['n_inp_ft'])
        if type_mdl == 'Conv1D':
            X_test = self.dict_test['X']
