...
class PsycopgMixin:
    ...
    def _remove_submit_parameter(self, url):
        return re.sub(r'([&?])submit=[^&]*(&|$)', r'\1\2', url)
    ...
    def search(self, form_args):
        ...
        except mentat.services.eventstorage.QueryCanceledException:
            query_canceled = True

        if query_canceled:
            self.abort(hawat.errors.error_response(499, message=self._remove_submit_parameter(flask.request.url)))
