...
class MenuBtn(QAbstractButton):
    ...
    def paintEvent(self, *a):
        ...
        hwnd = parent.hwnd()
        ISMAX, ISFULL = user32.IsZoomed(hwnd), parent.isFullScreen()
        ...
        isdisabled = (self.isminbtn and not hasminbtn) or (self.ismaxbtn and (not hasmaxbtn or not (resizable_h or resizable_v)))
...
class SystemLbl(QAbstractButton):
    ...
    def paintEvent(self, *a):
        ...
        painter.setPen(Qt.NoPen)
        painter.drawRect(self.rect())
        if (self.istitlebar or self.isclientarealbl) and NMAXFULL:
    ...
    def placeClientArea(self):
        ...
        title_h = self.parentattr('title_h')
        NMAXFULL = not (self.isMax() or parent.isFullScreen())
        parent.clientArea.setGeometry(QRect(*[bd_w * int(NMAXFULL), 0, parent.width() - bd_w * (2 if NMAXFULL else 0), parent.height() - title_h - bd_w * int(NMAXFULL)]))
...
state=0: All; state=1: Active; state=2: Inactive'''
    ...
    def setFixedSize(self, *a, **kwa):
        super(CustomizedWindow, self).setFixedSize(*a, **kwa)
        self.__resizable_h, self.__resizable_v = [False] * 2
    def setFixedWidth(self, *a, **kwa):
        super(CustomizedWindow, self).setFixedWidth(*a, **kwa)
        self.__resizable_h = False
    def setFixedHeight(self, *a, **kwa):
        super(CustomizedWindow, self).setFixedHeight(*a, **kwa)
...
|->||->|...'''
    ...
    def __BasicMH(self, hwnd, message, wParam, lParam):
        WM_SHOWWINDOW, WM_SETTINGCHANGE, WM_STYLECHANGED, WM_NCCALCSIZE, WM_NCHITTEST, WM_NCLBUTTONDOWN, WM_NCLBUTTONUP, WM_SYSCOMMAND, WM_DPICHANGED, WM_DWMCOMPOSITIONCHANGED = 0x18, 0x1a, 0x7d, 0x83, 0x84, 0xa1, 0xa2, 0x112, 0x2e0, 0x31e
        ...
        except: dpi, realdpi, real_bd_w, real_title_h, real_menubtn_w, mgn_l, mgn_t, mgn_r, mgn_b, resizable_h, resizable_v = [96] * 2 + [0] * 7 + [True] * 2
        resizable_hv = resizable_h and resizable_v
        windowrc = RECT()
        ...
        if message == WM_NCCALCSIZE:
            ...
            if ISMAX:
                ...
                if not list(filter(lambda n: n < 0, maxmgn_list)):
                    rc.left, rc.top, rc.right, rc.bottom = [getattr(bestrc, i[0]) for i in bestrc._fields_]
                    autohidetbpos = getautohidetbpos(rc)
                    autohidetbwidth_list = [2 if i == autohidetbpos and not ISFULL else 0 for i in range(4)]
                    rc.left += autohidetbwidth_list[0]
                    rc.top += autohidetbwidth_list[1]
                    rc.right -= autohidetbwidth_list[2]
                    rc.bottom -= autohidetbwidth_list[3]
                    f2 = lambda i: maxmgn_list[i] + autohidetbwidth_list[i]
                    self.__mgn_l, self.__mgn_t, self.__mgn_r, self.__mgn_b = map(f2, list(range(4)))
        ...
        if message == WM_NCHITTEST:
            ...
            if not 'res' in vars():
                if inminbtn:
                    ...
                    if self.__hasmaxbtn and (resizable_h or resizable_v):
