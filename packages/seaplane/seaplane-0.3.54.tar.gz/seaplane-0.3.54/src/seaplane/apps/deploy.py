...
def zip_current_directory(tenant: str) -> str:
    ...
    with zipfile.ZipFile(zip_filename, "w", zipfile.ZIP_DEFLATED) as zipf:         
        zipf.write("project.toml", os.path.relpath("project.toml", current_directory))
        zipf.write("requirements.txt", os.path.relpath("requirements.txt", current_directory))

        for root, _, files in os.walk(f"{current_directory}/src"):
            for file in files:                
