...
def to_gantt_json(pool: FjspPool, save_folder: str, machine_rate: dict,
    ...
    sorted_dict.get(pool.machine_dict[x].name[:2], 0), pool.machine_dict[x].name[-1])):
        ...
        for d in pd.date_range(min_day, max_day):
            calendar = (
                    [{"s_t": pd.to_datetime(d), "e_t": pd.to_datetime(d)}]
                    + machine_object.calendar.get(d, {}).get(
                "c_list", [{"s_t": pd.to_datetime(d), "e_t": pd.to_datetime(d) + timedelta(days=1)}])
                    + [{"s_t": pd.to_datetime(d) + timedelta(days=1), "e_t": pd.to_datetime(d) + timedelta(days=1)}]
            )
            for idx, c in enumerate(calendar):
                ...
                if e_t != s_t:
                    machine_dict["rest_time"].append({"start": s_t, "end": e_t})
        # 放入 operation 信息
