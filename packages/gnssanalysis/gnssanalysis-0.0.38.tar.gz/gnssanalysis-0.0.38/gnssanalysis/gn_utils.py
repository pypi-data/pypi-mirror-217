...
def clkq(input_clk_paths, norm, input_bia_paths, reject_re, plot):
    ...
    if reject_re is not None:
        reject_mask = diff_clk.columns.str.match(reject_re)
        _logging.info(msg=f"rejecting {diff_clk.columns[reject_mask].to_list()} according to '{reject_re}' expression")
        diff_clk = diff_clk.loc(axis=1)[~reject_mask]
