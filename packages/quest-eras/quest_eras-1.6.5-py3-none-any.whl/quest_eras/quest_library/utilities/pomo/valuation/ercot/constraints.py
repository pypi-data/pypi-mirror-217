...
def eq_objective_ercot_arbreg(m):
    """Formulate net revenue over the time horizon for ERCOT arbitrage and regulation."""
    mp = m.parent_block()
    ...
    _expr = sum(
        (
            ...
            + mp.price_reg_up[t] * mp.q_ru[t]
            + mp.price_reg_down[t] * mp.q_rd[t]
...
def eq_stateofcharge_ercot_arbreg(m):
    """Formulate state of charge for the storage device participating in ERCOT arbitrage and regulation."""
    mp = m.parent_block()
    ...
    def _eq_stateofcharge_ercot_arbreg(_m, t):
        return (
            ...
            + mp.Round_trip_efficiency * mp.q_r[t]
            - mp.q_d[t]
            + mp.Round_trip_efficiency * mp.fraction_reg_down[t] * mp.q_rd[t]
            - mp.fraction_reg_up[t] * mp.q_ru[t]
            == mp.s[t + 1]
        )

    m.stateofcharge = pom.Constraint(mp.time, rule=_eq_stateofcharge_ercot_arbreg)
...
def ineq_stateofcharge_minimum_reserve_twoprod(m):
    ...
    def _ineq_stateofcharge_minimum(_m, t):
        return (
            mp.s[t + 1]
            >= mp.Reserve_reg_min  # The number of hrs that storage device can at least discharge at q_ru rate.
            * mp.q_ru[t]
...
def ineq_stateofcharge_maximum_reserve_twoprod(m):
    ...
    def _ineq_stateofcharge_maximum(_m, t):
        return (
            mp.s[t + 1]
            <= mp.State_of_charge_max  # The number of hrs that storage device can at least charge at q_rd rate.
            * mp.Energy_capacity
            - mp.Round_trip_efficiency * mp.Reserve_reg_max * mp.q_rd[t]
        )
...
def ineq_power_limit_twoprod(m):
    ...
    m.power_limit = pom.Constraint(mp.time, rule=_ineq_power_limit_twoprod)