...
def eq_objective_pjm_pfp(m):
    """Formulate net revenue over the time horizon for pay-for-performance in PJM market."""
    mp = m.parent_block()
    ...
    _expr = sum(
        (
            ...
            + mp.q_reg[t]
            * mp.perf_score[t]
            * (mp.mi_mult[t] * mp.price_reg_service[t] + mp.price_regulation[t])
        )
        * math.e ** (-t * mp.R)
        for t in mp.time
    )
...
def eq_stateofcharge_pjm_pfp(m):
    """Formulate state of charge for storage device in PJM pay-for-performance market."""
    mp = m.parent_block()
    ...
    m.stateofcharge = pom.Constraint(mp.time, rule=_eq_stateofcharge_pjm_pfp)


def ineq_charge_limit_pjm_pfp(m):
    """Limit the energy charged at each timestep to the device maximum."""
    mp = m.parent_block()

    def _ineq_charge_limit_pjm_pfp(_m, t):
        return mp.Q_r_max >= mp.q_r[t] + mp.q_reg[t]

    m.charge_limit = pom.Constraint(mp.time, rule=_ineq_charge_limit_pjm_pfp)


def ineq_discharge_limit_pjm_pfp(m):
    """Limit the energy discharged at each timestep to the device maximum."""
    mp = m.parent_block()

    def _ineq_discharge_limit_pjm_pfp(_m, t):
        return mp.Q_d_max >= mp.q_d[t] + mp.q_reg[t]

    m.discharge_limit = pom.Constraint(mp.time, rule=_ineq_discharge_limit_pjm_pfp)
