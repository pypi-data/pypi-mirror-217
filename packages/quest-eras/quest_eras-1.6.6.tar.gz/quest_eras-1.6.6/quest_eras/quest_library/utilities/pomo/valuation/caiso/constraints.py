...
def eq_objective_caiso_pfp(m):
    """Formulate net revenue over the time horizon for pay-for-performance in the CAISO market."""
    mp = m.parent_block()
...
def eq_stateofcharge_caiso_pfp(m):
    """Formulate state of charge for storage device in CAISO market."""
    mp = m.parent_block()
    ...
    m.stateofcharge = pom.Constraint(mp.time, rule=_eq_stateofcharge_caiso_pfp)
