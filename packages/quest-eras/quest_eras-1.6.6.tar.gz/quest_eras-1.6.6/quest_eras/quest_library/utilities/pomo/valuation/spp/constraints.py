...
def eq_objective_spp_pfp(m):
    """Formulate net revenue over the time horizon for pay-for-performance in the SPP market."""
    mp = m.parent_block()
...
def eq_stateofcharge_spp_pfp(m):
    """Formulate state of charge for storage device in SPP market."""
    mp = m.parent_block()
    ...
    m.stateofcharge = pom.Constraint(mp.time, rule=_eq_stateofcharge_spp_pfp)
