...
from utilities.pomo.valuation.constraints import ExpressionsBlock
import logging
...
class ValuationOptimizerERCOT(optimizer.Optimizer):
    ...
    def _process_results(self):
        ...
        if self.market_type == "ercot_arbreg":
            rev_arb = np.cumsum(
                np.array(
                    [
                        m.price_electricity[t] * (m.q_d[t].value - m.q_r[t].value)
