...
from os.path import abspath, exists, join, expanduser, basename, dirname, isdir
...
BUILD_FLOW = ["{type}", "authenticate-{type}-{host}", "push-{type}", "cleanup-{type}"]
...
def from_config_file(package_directory: str = None):
    ...
    if not exists(join(package_directory, "maintenance.yaml")):
    ...
    meta = safe_load(open(join(package_directory, "maintenance.yaml"), "r"))
    for step in BUILD_FLOW:
...
@impose
def authenticate_python_package_codeartifact(domain: str = None, repo: str = None, region: str = None):
