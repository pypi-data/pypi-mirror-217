...
def upload_new_experiment(module_dir, repo_path, testing, testing_value, paired_end_test, experiment_column):
    ...
    if choose_method == "1":
        ...
        sequencing_report, all_alignment_reports = method_three(module_dir, experiment, testing)
    else:
...
def retrieve_reports(module_dir, experiment, testing):
    if not testing:
        seq_report_path = os.path.join(module_dir, "my_experiments", experiment, "sequencing_report.txt")
        align_repo_path = os.path.join(module_dir,
                            "my_experiments",
                            experiment,
                            "alignment_reports", "*")
