...
def load_yaml(path):
    try:
        ...
        error(f"Invalid plan file: {str(exc)}")
...
def handle_add(cmdargs, _, config):
    ...
    if cmdargs.config:
        name, config_data = load_yaml(cmdargs.config)
        data["config"] = config_data

    if cmdargs.plan:
        if os.path.isfile(cmdargs.plan):
            name, plan_data = load_yaml(cmdargs.plan)
            ...
            for plan in files:
                name, plan_data = load_yaml(plan)
                data["plans"].append((name, plan_data))

    ret = post(config, url, data=data)

    if ret.status_code != 201:
        error("Failed to add tuxtrigger 'config/plan'")
    else:
