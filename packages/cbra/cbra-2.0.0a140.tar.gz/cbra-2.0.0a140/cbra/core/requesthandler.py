...
class RequestHandler(Generic[E]):
    ...
    def add_to_class(self, cls: type[IEndpoint]) -> None:
        ...
        empty = [
            p.replace(kind=Parameter.POSITIONAL_ONLY)
            for p in parameters if p.default == Parameter.empty
        ]
        non_empty: filter[Parameter] = filter(lambda p: p.default != Parameter.empty, parameters)
        parameters = [
            *filter(lambda p: p.kind == Parameter.POSITIONAL_ONLY, empty),
            *sorted(non_empty, key=lambda x: x.kind)
        ]
    ...
    async def _run_handler(self, **params: Any) -> Any:
        ...
        endpoint: IEndpoint = self._class(**init) # type: ignore
