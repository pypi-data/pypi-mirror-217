    ...
    def __init__(self, *,
        ...
        if not isinstance(partition_name, str):
            raise CentaurException(code=CentaurCode.InvalidArgument,
                                   reason=f"CentaurSDK DropPartitionRequest name Type({partition_name}) is Invalid")

        if len(partition_name) < 3 or len(partition_name) > 32:
            raise CentaurException(code=CentaurCode.InvalidArgument,
                                   reason=f"CentaurSDK DropPartitionRequest name Length({len(partition_name)}) is Invalid and must be in [3, 32]")

        if re.search('^[a-zA-Z][a-zA-Z0-9_]{0,32}$', partition_name) is None:
            raise CentaurException(code=CentaurCode.InvalidArgument,
                                   reason=f"CentaurSDK DropPartitionRequest name Characters({partition_name}) is Invalid and must be in [a-zA-Z0-9] and symbols[_]")
