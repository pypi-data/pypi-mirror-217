...
class RoleApiServices():
    ...
    def get_apis(params: dict = None, only_first: int = None, pagination=True, **kwargs):
        ...
        sort = sort if sort in ["id", "-id", "-created_time", "created_time"] else "-created_time"

        params = format_params_handle(
            param_dict=params,
            filter_filed_list=["id|int", "route|str", "name|str", "is_using|int"],
            alias_dict={
                "route": "route__contains",
                "name": "name__contains"
            }
        )
    ...
    def add_api(params: dict = None, **kwargs):
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=["route|str", "method|str", "name|str", "description|str"]
        )
    ...
    def edit_api(pk: int, params: dict = None, **kwargs):
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=["route|str", "method|str", "name|str", "description|str", "is_using|int"]
        )
...
class requestSwitch(object):
    def request_switch(self, *args, current_url: str = "", request_params: dict = None, user_info=None, **kwargs):
        """
        请求API请求锁
        @description: 拦截当前用户的API请求，获取当前用户的角色，并校验是否有权限请求该接口，如果有权限的继续校验请求参数
        :return: data ,err
        """
        current_url, err = force_transform_type(variable=current_url, var_type="str", default="")
