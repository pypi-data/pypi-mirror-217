...
class PermissionValueService():
    ...
    def add_permission(params: dict = None, **kwargs):
        ...
        try:
            ...
            if not params.get("role_id") or not Role.objects.filter(id=params.get("role_id"), is_using=1, is_delete=0).first():
                return None, "msg:不是一个有效的角色ID（role_id）;tip:不是一个有效的角色ID"

            if params.get("type") == "API" and (not params.get("api_id") or not RoleApi.objects.filter(id=params.get("api_id"), is_using=1, is_delete=0).first()):
                return None, "msg:不是一个有效的接口ID（api_id）;tip:不是一个有效的接口ID"

            if params.get("type") == "MENU" and (not params.get("menu_id") or not RoleMenu.objects.filter(id=params.get("menu_id"), is_using=1, is_delete=0).first()):
