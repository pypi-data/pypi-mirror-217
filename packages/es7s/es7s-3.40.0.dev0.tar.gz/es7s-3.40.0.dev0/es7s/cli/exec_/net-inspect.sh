...
_main() {
            ...
            i|interface) iface="$OPTARG" ;;
               a|action) start_mode="$OPTARG" ;;
                 h|help) help_invoked=true ;;
                    ??*) echo "1Illegal option -${OPTARG:--$OPT}. Usage: $__USAGE"
                         exit 1 ;;
                      ?) echo "2Illegal option -${OPTARG:--$OPT}. Usage: $__USAGE"
                         exit 1 ;;
            esac
        done
        shift $((OPTIND-1))
    ...
    _menuitem() {
        ...
        printf " $(_menubtn "$key") \x1b[22;32m${prefix}%-${DESC_WIDTH}s${suffix}\x1c\x1b[34m$cmd\x1b[m\n" "$desc" |
          sed --unbuffered -Ee "s/sudo/\x1b[2m&\x1b[22m/g; s/$iface/\x1b[36m&\x1b[34m/g; s/&(\w)/\x1b[1;33m\1\x1b[22;32m/; Tb; s/\x1c/ /; :b s/\x1c//;"
    }
    _menubtn() {
        local key=$1
        local bfmt=39 cmd=${CMDS[$key]}
        if [[ -n "$cmd" ]] ; then
            [[ "$cmd" =~ ^_ ]] && bfmt=32 || bfmt=33
        else key=" " ; fi
    ...
    _usage() {
        ...
        _menuitem i "&interfaces summary"
    ...
    if [[ -n $header ]] ; then
        printf "\x1b[37mes7s/net-inspect ${VERSION}\x1b[m\n"
    ...
    while true ; do
        ...
        case "${resp::1}" in
           i|l|m|h|p|t|u|q|\?) cmd=${CMDS[${resp::1}]}
                               resp="${resp:1}"
                               printf "\x1b[0K"
                               [[ -z $one_time ]] && printf " "
                               ...
                               redraw=1
                               ;;
            ??*|?) echo -ne "\x1b[0K $helphint \x1b[31m Invalid action: \x1b[91m${resp::1}\x1b[39m\r"
                   resp=
                   redraw=
                   ;;
        esac
