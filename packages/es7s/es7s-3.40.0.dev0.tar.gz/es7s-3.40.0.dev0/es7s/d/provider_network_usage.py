...
from es7s.shared.statistics import median
...
class NetworkUsageProvider(DataProvider[NetworkUsageInfo]):
    ...
    def _collect(self) -> NetworkUsageInfo:
        ...
        snic: snicstats = psutil.net_if_stats().get(self._interface, None)
        if not snic:
            return NetworkUsageInfo(self._interface)

        cdict: dict[str, snetio] = t.cast(dict, psutil.net_io_counters(pernic=True, nowrap=True))
        counters: snetio = cdict.get(self._interface, None)
        if not counters or not snic.isup:
            return NetworkUsageInfo(self._interface, snic.isup)
        ...
        for ds in [*self._data_queue, ds_now]:
            ...
            for bytesn, drop, err, idc in [
                ...
                idc.drops += drop
                idc.errors += err
                self._max_bps = max(self._max_bps, bps)
        ...
        return NetworkUsageInfo(self._interface, snic.isup, *stats())
