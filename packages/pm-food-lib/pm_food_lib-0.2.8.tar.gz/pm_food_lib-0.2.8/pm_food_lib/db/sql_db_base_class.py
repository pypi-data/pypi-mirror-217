...
from pm_food_lib.utils.constants.log_levels import LogLevels
    ...
    def instance(cls, host, port, user, password, database, pool_name, pm_logger, pool_size=10):
        if cls._instance is None:
            ...
            pool = pooling.MySQLConnectionPool(
                pool_name=pool_name, pool_size=pool_size, pool_reset_session=True, **res
            )
            cls.pool = pool
    ...
    def database_error_handling_to_log(self, error, sql_statement):
        ...
        if "errno" in error.__dict__:
            ...
            if error.errno == errorcode.ER_BAD_TABLE_ERROR:
                log_message = {"status": "BAD", "error_message": f"Error : {error}", "sql_statement": sql_statement}
