...
from pm_food_lib.utils.functions.general import generate_uid
...
def get_pm_options_format(option_id, name: str = None, price: str = None):
    item = {"id": option_id, "name": name, "price": price}

    return item


def get_pm_extra_ingredients_format(extra_ingredient_id, name: str = None, price: str = None):
    item = {"id": extra_ingredient_id, "name": name, "price": price}

    return item


def get_pm_removed_ingredients_format(removed_ingredient_id, name: str = None):
    item = {"id": removed_ingredient_id, "name": name}

    return item


def get_pm_order_products_format(
    product_id,
    name: str = None,
    quantity: str = None,
    note: str = None,
...
def get_pm_promotions_format(promotion_id, name: str = None):
    ...
    promotions = {"id": promotion_id, "name": name}
    return promotions
...
def get_pm_dynamodb_order_format():
    ...
    item = {
        ...
        "payments": {
            ...
            "methods": [],
            "cancelation_info": {"description": None, "option": None},
        ...
        "courier_app_operation": {
            "on_the_way": {"lat": None, "long": None, "created_at": None},
            "delivered": {"lat": None, "long": None, "created_at": None},
            "notdelivered": {"lat": None, "long": None, "created_at": None},
        },
        "is_dirty_payment": "0",
        "total_price": None,
        "discount_price": "0.00",
...
def get_constant_cancel_options():
    cancel_options_json = {
        "cancel_options": [
            {"id": "3", "name": "Elektrikler kesildi."},
            {"id": "2", "name": "Malzemeler eksik."},
            {"id": "1", "name": "Müşteri istemedi."},
        ]
    }
