...
from pm_food_lib.utils.constants.error_codes import MessageCode
import re
import jwt
import math
from pm_food_lib.utils.constants.PM import BillingStatus, OrderStatus
from pm_food_lib.utils.constants.parameters import (
    PAREKENDE_SATIS_FISI_LIMIT,
    TOLERANCE_PAYMENT,
    DYNAMO_DB_NULL_INDEX,
    PAYMENT_PER_BASKET,
)
from pm_food_lib.utils.constants.platforms import BrandsPlatformStatus
from math import sin, radians, cos, asin, sqrt


# TODO: datetime eklemeye gerek yok kaldirilabilir
def generate_uid():
    return datetime.now().strftime("%Y%m-%d%H-%M%S-") + str(uuid4())


def init_extra_log_params(
    log_id,
    sql_error_code=None,
    sql_statement=None,
    db_name=None,
...
def date_format_checker(dates):
    if (
        dates.get("start_date") is None
        or dates.get("start_date") == ""
        or dates.get("end_date") is None
        or dates.get("end_date") == ""
    ):
        return False, MessageCode.MISSING_DATA_ERROR_MESSAGE

    if not isinstance(dates.get("start_date"), str) or not isinstance(dates.get("end_date"), str):
        return False, MessageCode.DATE_FORMAT_ERROR

    try:
        if not bool(datetime.strptime(dates.get("start_date"), "%Y-%m-%d")) or not bool(
            datetime.strptime(dates.get("end_date"), "%Y-%m-%d")
        ):
            return False, MessageCode.DATE_FORMAT_ERROR
    except Exception:
        return False, MessageCode.DATE_FORMAT_ERROR
...
def generate_token():
    ...
    encoded = jwt.encode(
        {"name": "Emre", "surname": "Ozgun", "exp": datetime.utcnow() + timedelta(hours=24)},
        private_key,
        algorithm="RS256",
    )
...
def find_distance_between_points(point_1, point_2):
    try:
        distance = math.sqrt(
            (float(point_1[0]) - float(point_2[0])) ** 2 + (float(point_1[1]) - float(point_2[1])) ** 2
        )
    except Exception:
        return None
    else:
    ...
    if edited_payments and len(edited_payments) >= 1:
        ...
        for payment in payments:
            ...
            if payment["name"] in current_payment_obj:
                ...
                current_payment_obj[payment["name"]] = {
                    "payment": payment["name"],
                    "platform": row["platform_name"],
                    "total_price": float(payment["price"]),
                }
    else:
        current_payment_obj["WRONG_ARCHIVE_DATA"] = {
            "payment": "WRONG_PAYMENT_TYPE",
            "platform": row["platform_name"],
...
def check_payments_price_with_total_price(payment_methods: dict, total_price: float):
    try:
        ...
        return (
            [True, "OK"]
            if float(abs(float(payments_total_price) - float(total_price))) <= TOLERANCE_PAYMENT
            else [False, msg]
        )
...
def order_type_format_checker(order_type):
    ...
    if (
        ...
        and order_type != OrderStatus.PREPARING
        and order_type != OrderStatus.PREPARED
    ):
...
def update_order_status_format_checker(order_status):
    ...
    if (
        ...
        and order_status != OrderStatus.PREPARING
        and order_status != OrderStatus.PREPARED
...
def rest_open_close_format_checker(rest_status):
    ...
    if rest_status != BrandsPlatformStatus.OPEN and rest_status != BrandsPlatformStatus.CLOSE:
