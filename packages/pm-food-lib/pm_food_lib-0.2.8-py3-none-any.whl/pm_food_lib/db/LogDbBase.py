...
from pm_food_lib.db.sql_db_base_class import PmMysqlBaseClass
    ...
    def insert_log(
        ...
        try:
            ...
            if type(conn) == dict and conn.get("status_code") == "BAD":
                respond_error_data = {
                    "log_id": conn.get("log_id"),
                    "message_code": MessageCode.UNEXPECTED_ERROR_ON_SERVICE_MESSAGE,
                }

                return respond_error_data, 500

            insert_service_logs_query = (
                "INSERT INTO Service_Logs (id, service_name, message, short_message, "
            ...
            respond_record = self.execute_without_commit(
                conn=conn, cursor=cursor, sql=insert_service_logs_query, args=record_args
            )
