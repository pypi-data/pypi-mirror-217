...
class RsCMPX_LteMeas:
	...
	_driver_options = "SupportedInstrModels = CMX500/CMP/CMW/PVT, SupportedIdnPatterns = CMW/CMP/CMX/PVT, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,4.0.185.0011'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.185.0011'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.185.0011'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_LteMeas version failed. Current version: '4.0.185.0011', minimum required version: '{min_version}'")
