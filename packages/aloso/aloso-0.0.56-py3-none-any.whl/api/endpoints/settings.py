...
async def add_equipe(data: dict):
    response_content = {"message": "Un problème est survenu lors de l'ajout de l'equipe"}
    response_status = 500
    if data['name'] != "":
        equipe = Equipes( name=data['name'])
        equipe.save()
        response_content = {"message": "Equipe créé avec succès !"}
        response_status = 200
...
async def delete_equipes(data: dict):
    response_content = {"message": "Un problème est survenu lors de la suppression de l'equipe"}
    response_status = 500
    equipe = Equipes.get_by_id(data['idEquipe'])
    if equipe:
        equipe.delete()
        response_content = {"message": "Equipe supprimé avec succès"}
        response_status = 200
...
async def modify_equipe(data: dict):
    response_content = {"message": "Un problème est survenu lors de la modification de l'equipe"}
    response_status = 500
    if data['name'] != '':
        equipe = Equipes.get_by_id(data['idEquipe'])
        equipe.name = data['name']
        equipe.save()
        response_content = {"message": "Equipe modifié avec succès"}
        response_status = 200
...
async def link_user_teams(data: dict):
    ...
    response_content = {"message": "Un problème est survenu lors de la mise à jour des liens"}
    response_status = 500
    list_teams = []
    ...
    if user_id:
        ...
        if user.link_unlink_base(second_relation_objects=list_teams, method=user.toggle_user_equipe_link): #if user.update_user_teams_link(equipes=list_teams):
            response_content = {"message": "Opération effectuée avec succès !"}
            response_status = 200
...
@router.post("/equipments/connections")
