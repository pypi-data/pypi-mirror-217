...
class SaveUrlView(APIView):
    def get(self, request):
        ...
        if url is None:
            ...
            input_kwargs.update({"extractors": 'singlefile'})
            add_stdout = StringIO()
            with redirect_stdout(add_stdout):
                add(**input_kwargs)
                print(add_stdout.getvalue())

            # return folder name / singlefile.html path
            return Response({'msg': 'OK'})


class ReadIndexView(APIView):
    def get(self, request):
        url = request.query_params.get('url', None)
        print('url: {}'.format(url))
        if url is None:
            ...
            qs = query_search_index(url)
            for item in qs:
                print('qs: {}'.format(item))
                if item.is_archived:
                    fpath = "archive/{}/singlefile.html".format(item.timestamp)
                    # check if file exist?
                    print("filepath: {}".format(os.path.join(OUTPUT_DIR, fpath)))
                    if os.path.exists(os.path.join(OUTPUT_DIR, fpath)):
                        return Response({'fpath': fpath})
