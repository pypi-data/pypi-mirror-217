...
inline __device__ void compute_dq_dk_dv_1Nblock(const Params &params, const int bidb, const int bidh, Prng &ph, const int n_block) {
    ...
    const uint32_t row_offset_q = (binfo.sum_s_q + (m_block_max - 1) * kBlockM) * params.q_row_stride + bidh * params.q_head_stride;
    const uint32_t row_offset_k = (binfo.sum_s_k + n_block * kBlockN) * params.k_row_stride + bidh * params.k_head_stride;
    const uint32_t row_offset_v = (binfo.sum_s_k + n_block * kBlockN) * params.v_row_stride + bidh * params.v_head_stride;
    const uint32_t row_offset_do = (binfo.sum_s_q + (m_block_max - 1) * kBlockM) * params.do_row_stride + bidh * params.do_head_stride;
    const uint32_t row_offset_o = (binfo.sum_s_q + (m_block_max - 1) * kBlockM) * params.o_row_stride + bidh * params.o_head_stride;
    // We'll decrease dQ before the 1st read
    const uint32_t row_offset_dq = (binfo.sum_s_q + m_block_max * kBlockM) * params.dq_row_stride + bidh * params.dq_head_stride;
    ...
    Tensor gdQ = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.dq_ptr) + row_offset_dq - kBlockM * params.dq_row_stride),
                             Shape<Int<kBlockM>, Int<kHeadDim>>{},
    ...
    const uint32_t row_offset_dk = (binfo.sum_s_k + n_block * kBlockN) * params.dk_row_stride + bidh * params.dk_head_stride;
    const uint32_t row_offset_dv = (binfo.sum_s_k + n_block * kBlockN) * params.dv_row_stride + bidh * params.dv_head_stride;
