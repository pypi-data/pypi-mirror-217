...
def finetune_runner(component_args: Namespace) -> None:
    ...
    if component_args.model_family == MODEL_FAMILY_CLS.MMDETECTION_IMAGE:
        # gathering from all the processes to set the max_no_labels_per_image
        for ds in [data_cls.get_train_dataset(), data_cls.get_validation_dataset()]:
            maximum_no_labels_per_image = ds.maximum_no_labels_per_image
            maximum_no_labels_per_image_list = distributed_utils.all_gather(maximum_no_labels_per_image)
            ds.maximum_no_labels_per_image = max(maximum_no_labels_per_image_list)
