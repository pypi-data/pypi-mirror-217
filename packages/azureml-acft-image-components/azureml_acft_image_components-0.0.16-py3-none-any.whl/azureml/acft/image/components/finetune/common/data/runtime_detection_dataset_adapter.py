...
class RuntimeDetectionDatasetAdapter(CommonObjectDetectionDatasetWrapper):
    ...
    def __init__(self, dataset: CommonObjectDetectionDataset) -> None:
        ...
        # getting the max no of labels per image for padding.
        # distributed training expects the same length while gathering.
        max_no_labels = 0
        for data_ele in self._dataset_elements:
            cur_no_labels = len(self._dataset.get_image_label_info(data_ele)[1]['labels'])
            max_no_labels = max(max_no_labels, cur_no_labels)
        self.maximum_no_labels_per_image = torch.tensor(max_no_labels)
