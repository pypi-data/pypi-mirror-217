...
class CuSum_:
    ...
    def __init__(self, path, date_start, date_end, study_site):
        ...
        #     pass
        # finally:
        #     out_dl = os.path.join(self.path, "Downloaded_img")

        if not os.path.exists(os.path.join(self.path, "Downloaded_img")):
            os.mkdir(os.path.join(self.path, "Downloaded_img"))
            out_dl = os.path.join(self.path, "Downloaded_img")
        ...
        if not os.path.exists(os.path.join(self.path, "zone_files")):
            os.mkdir(os.path.join(self.path, "zone_files"))
            out_zone = os.path.join(self.path, "zone_files")
        else:
            out_zone = os.path.join(self.path, "zone_files")
        ...
        if not os.path.exists(os.path.join(self.path, "Pre_processed_img")):
            os.mkdir(os.path.join(path, "Pre_processed_img"))
            out_pre = os.path.join(path, "Pre_processed_img")

        else:
            out_pre = os.path.join(path, "Pre_processed_img")
        self.out_pre = out_pre

        date1 = "".join(date_start.split('-'))
        date2 = "".join(date_end.split('-'))
        if not os.path.exists(os.path.join(self.path, "Output_CuSum" + "_" + date1 + "_" + date2)):
            os.mkdir(os.path.join(path, "Output_CuSum" + "_" + date1 + "_" + date2))
            out_algo = os.path.join(path, "Output_CuSum" + "_" + date1 + "_" + date2)

        else:
            out_algo = os.path.join(path,  "Output_CuSum" + "_" + date1 + "_" + date2)
        self.out_algo = out_algo
    ...
    def post_processing(self, th, tl, method, mf_shp, area_th=300, area_tl=1000, nb_max=10):
        ...
        for vh, vv in zip(high_vh_list, high_vv_list):
            ...
            if method == 'multi':
                ...
                remove_high_nb_change(self.out_algo, cross_nb(self.out_algo, vh, vv, mf_shp),
