...
"""
    ...
    def _validate_join(self, f: Callable):
        ...
        if join.lhs.converter.primitive_type != join.rhs.converter.primitive_type:
            raise TypeError(
                f"Join function for {self.root_fqn} is incorrectly configured. "
                f"Ensure that both sides of the join are the same type (either int or str). "
                f"'{join.lhs.root_fqn}' is of type {join.lhs.typ.parsed_annotation}, and "
                f"'{join.rhs.root_fqn}' is of type {join.rhs.typ.parsed_annotation}. "
            )
