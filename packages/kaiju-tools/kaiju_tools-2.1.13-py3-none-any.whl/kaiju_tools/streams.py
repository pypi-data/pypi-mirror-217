...
from kaiju_tools.interfaces import Locks, SessionInterface, AuthenticationInterface, Stream
...
class StreamRPCClient(BaseRPCClient, abc.ABC):
    ...
    async def init(self):
        self._transport = self.discover_service(self._transport, cls=Listener)
        await super().init()
    ...
    async def call(
        ...
        if headers is None:
            headers = {}
        headers['_topic'] = Namespace._create_key(self.app.env, app, '_stream', topic)  # TODO
