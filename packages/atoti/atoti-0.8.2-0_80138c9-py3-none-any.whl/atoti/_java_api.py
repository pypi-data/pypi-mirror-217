...
def _convert_java_mapping_to_combined_isin_constant_condition(
    mapping: Mapping[str, Any], /, *, identify: Callable[[str], IdentifierT]
) -> Optional[_CombinedIsinConstantCondition[IdentifierT]]:
    if not mapping:
        return None
...
class JavaApi(metaclass=ApiMetaClass):
    ...
    def _convert_combined_isin_condition_constant_condition_to_java_map(
        self,
        condition: Optional[_CombinedIsinConstantCondition[IdentifierT]],
        /,
        *,
        identifier_type: type[IdentifierT],  # pyright: ignore[reportGeneralTypeIssues]
        get_key: Callable[[IdentifierT], str],
    ) -> JavaMap:
        result: dict[IdentifierT, tuple[Constant, ...]] = {}
    ...
    def _convert_python_aggregate_provider_to_java(
        ...
        java_measures = to_java_object_list(
            [
                measure_identifier.java_description
    ...
    def create_cube_from_table(
        ...
        *,
        ...
        self._outside_transaction_api().createCubeFromStore(
            table_identifier.table_name, cube_name, creation_mode
        )
