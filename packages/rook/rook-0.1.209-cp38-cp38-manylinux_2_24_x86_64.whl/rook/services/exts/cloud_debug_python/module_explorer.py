      ...
      for obj in referents:
        try:
          ...
          if isinstance(obj, types.CodeType) and CheckIgnoreCodeObject(obj):
            logger.dev_debug("Ignoring code object with name: {0}".format(obj.co_name))
            if not hasattr(module, '__file__'):
              logger.dev_debug("    module doesn't have __file__ attribute")
            else:
              code_object_file = os.path.splitext(obj.co_filename)[0]
              module_file, module_file_ext = os.path.splitext(module.__file__)
              logger.dev_debug("    code_object_file = {0}".format(code_object_file))
              logger.dev_debug("    module_file, module_file_ext = {0}, {1}".format(module_file, module_file_ext))
              logger.dev_debug("    basename of paths: {0} --- {1}".format(os.path.basename(code_object_file), os.path.basename(module_file)))
            continue
          ...
          if isinstance(obj, six.class_types) and CheckIgnoreClass(obj):
            logger.dev_debug("Ignoring class with name: {0}".format(obj.__name__))
            if not hasattr(module, '__file__'):
              logger.dev_debug("    module doesn't have __file__ attribute")
            else:
              cls_module = sys.modules.get(obj.__module__)
              logger.dev_debug("    Is class module None? {0}".format(cls_module is None))
              logger.dev_debug("    Is class module the same as module? {0}".format(cls_module is module))
              logger.dev_debug("    Is class module the same as module (using __file__)? {0}".format(getattr(cls_module, '__file__', None) == module.__file__))
            continue
