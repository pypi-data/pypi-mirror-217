...
class BytecodeBreakpoint {
 public:
  ...
  int SetBreakpoint(
      ...
      std::function<void()> error_callback, PyObject** o_new_co_code);

  // Removes a previously set breakpoint. If the cookie is invalid, this
  // function does nothing.
  void ClearBreakpoint(int cookie, PyObject** o_new_co_code);
 ...
 private:
  ...
  struct CodeObjectBreakpoints {
    ...
    // Original value of PythonCode::co_exceptiontable before patching.
    ScopedPyObject original_exception_table;
  };
  ...
  // is idempotent.
  void PatchCodeObject(CodeObjectBreakpoints* code, PyObject** o_new_co_code);
