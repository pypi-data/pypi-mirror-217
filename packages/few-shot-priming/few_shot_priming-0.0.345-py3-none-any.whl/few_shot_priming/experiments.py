...
def oversample_dataset(df, experiment):
    if experiment == "ibmsc":
        ...
        count_of_con_to_sample = pro_claims.shape[0] - con_claims.shape[0]
        count_of_neutral_to_sample = pro_claims.shape[0] - neutral_claims.shape[0]
        con_claims_to_add = con_claims.sample(count_of_con_to_sample)
        neural_claims_to_add = neutral_claims.sample(count_of_neutral_to_sample)
        return pd.concat([df, con_claims_to_add, neural_claims_to_add])
