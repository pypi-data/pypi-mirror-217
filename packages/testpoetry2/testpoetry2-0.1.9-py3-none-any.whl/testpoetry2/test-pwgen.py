...
OPTIONS_ARGUMENTS = {
    ...
    'f_err': {
        ...
        'message': 'Which file would you like to use?',
        'validation': lambda val: True if val else 'Please, provide a path to the file',
        'default': './pattern-list-error.txt'
    },
    'c': {
        'descr': 'c',
        'type': 'text',
    ...
    'pattern': {
        ...
        'options': {
            ...
            'pattern_custom_charset_exclusions': {
                ...
                'command': 'python pwgen.py -t "u{{4}}[Ld^l^\\4^\\5^\\6^\\7^\\8]{{3}}l{{2}}" -c{} {} {}',
                'arguments': [OPTIONS_ARGUMENTS['c'], OPTIONS_ARGUMENTS['p'], OPTIONS_ARGUMENTS['v'], ],
            },
            'pattern_complex': {
                'descr': 'complex pattern',
                'command':
                    'python pwgen.py -t "u{{4}}\\-[Ld^l^\\4^\\5^\\6^\\7^\\8\\@\\$\\%\\&\\#\\*\\!]{{3}}l{{2}}" -c{} {} {}',
                'arguments': [OPTIONS_ARGUMENTS['c'], OPTIONS_ARGUMENTS['p'], OPTIONS_ARGUMENTS['v'], ],
            },
        },
    },
    ...
    'pipe': {
        ...
        'options': {
            ...
            'pipe_stderr': {
                ...
                'command': 'python pwgen.py -S "Ld^l^\\4^\\5^\\6^\\7^\\8" {} 2> {}',
                'arguments': [OPTIONS_ARGUMENTS['v'], OPTIONS_ARGUMENTS['f_log'], ],
            },
            'pipe_complex': {
                'descr': 'complex pipe with a sort command',
                'command': 'echo u{{4}}[Ld^l^\\4^\\5^\\6^\\7^\\8]{{3}}l{{2}} | python pwgen.py -c6 {} | sort -r ',
    ...
    'with_errors': {
        ...
        'options': {
            'charset_error': {
                ...
                'command': 'python pwgen.py -S "Ld^lwkr^\\4^\\5^\\6^\\7^\\8" {}',
                'arguments': [OPTIONS_ARGUMENTS['v'], ],
            },
            'pattern_error': {
                'descr': 'pattern with an error placeholder',
                'command': 'python pwgen.py -t "{{5}}Ld^l^\\4^\\5^\\6^\\7^\\8" {}',
                'arguments': [OPTIONS_ARGUMENTS['v'], ],
            },
            'file_error': {
                'descr': 'pattern with errors from a file',
                'command': 'python pwgen.py -f {} {}',
...
def main():
    ...
    command = ''.join(
        [
            *[o_val.get('command', '').format(*[a_val for _, a_val in list(answers.items())[2:]])
            ...
            *[val.get('command', '').format(*[a_val for _, a_val in list(answers.items())[1:]])
              for key, val in OPTIONS.items()
              if key == answers.get('category', None) and 'arguments' in val and 'command' in val
              ],
        ]
    )
