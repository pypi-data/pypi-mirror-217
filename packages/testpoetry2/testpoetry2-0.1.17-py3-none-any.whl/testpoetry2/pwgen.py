...
def generate_password_from_pattern(pattern: str) -> str:
    ...
    return ''.join(random.choice(el) for el in pat_ls)
...
def generate_passwords(length: int, count: int, template: Optional[str] = None,
                       file: Optional[str] = None, character_set: Optional[str] = None,
                       permute: Optional[bool] = False) -> None:
    passwords = []
    if template:
        ...
        logger.info(f'Let\'s generate {count} passwords based on a charset {"".join(sorted(character_set))}')

        passwords = [generate_password_from_character_set(length, character_set) for _ in range(count)]
