        ...
        if 'import_service' in node or 'import_connection_name' in node:
            ...
            if service == 's3':
                ...
                if cron == AUTO_CRON and current_ws:
                    ...
                    rate_limits: Dict[str, Dict[str, int]] = next((w['rate_limits'] for w in workspaces if w['id'] == current_ws['id']), {})
                    period: int = rate_limits.get('api_datasources_create_append_replace', {}).get('period', 60)

                    def seconds_to_cron_expression(seconds: int):
                        minutes = seconds // 60
                        hours = minutes // 60
                        days = hours // 24
                        if days > 0:
...
async def new_ds(
    ...
    if service and service in ['bigquery', 'snowflake', 's3']:
        try:
