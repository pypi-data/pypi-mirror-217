...
class timeSeriesVisualization():
    ...
    def additive_decomposition(self) -> tuple:
        ...
        additive_decomp = seasonal_decompose(
            y, model="additive", period=30, extrapolate_trend="freq"
        )

        plt.rcParams.update({"figure.figsize": (16, 16)})
        ax = additive_decomp.plot().suptitle("Additive Decomposition", fontsize=16)
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.xlabel("Giorni")
        # plt.show()

        return (additive_decomp.trend, additive_decomp.seasonal, ax.get_figure())
    ...
    def boxplots_seasons(self) -> plt.figure:
        ...
        df_ = self.df_.set_index(self.data).copy()
        df_ = df_.copy()
        df_["date"] = df_.index
        ...
        df_plot = df_[["weekday", "season", target]]
    ...
    def analyzingTimeSeries(self, aggregation_level: str,) -> dict:
        ...
        if len(self.df) > 5:
            ...
            autocorr = self.autocorrelation(n_lags=len(self.df.dropna()[self.target]) // 3 - 1)
