...
class Pairplot(object):
    ...
    def on_plot(self, isReload=False):
        ...
        # Mask on dataframe columns that contain string values 
        _mask = [self.is_string_series(self.signal_df[col]) for col in self.signal_df.columns.tolist()]
        string_cols = self.signal_df.columns[_mask].tolist()
        nonstring_cols = list(set(self.signal_df.columns.tolist()) - set(string_cols))
        ...
        if not nonstring_cols:
            ...
            # Check if columns with strings exist and colour by accordingly
            if len(string_cols) > 1:
                with self.output:
                    ...
                    sep = widgets.VBox(children=[widgets.HTML(value = _sep, layout=self.layout),
                                             widgets.HTML(value = "<h1>Pairplot of selected signals</h1>")])
                    display(sep)

                    if not isReload:
                        self.colour_by_selection = self.colour_by.value

                sns.pairplot(self.signal_df, hue=self.colour_by_selection)
            elif len(string_cols) == 1:
                sns.pairplot(self.signal_df, hue=string_cols[0])
            else:
                sns.pairplot(self.signal_df)
            plt.show()
        ...
        _sep = markdown.markdown('***', extensions=['markdown.extensions.tables'])
        sep = widgets.VBox(children=[widgets.HTML(value = _sep, layout=self.layout),
                                 widgets.HTML(value = "<h1>Lineplot of selected signals</h1>")])
        display(sep)
