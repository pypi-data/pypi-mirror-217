...
class Polygon:
    ...
    def is_between(self,
        ...
        if dst is not None:
            ...
            dst = Location(src.x, src.y).move(theta, dist)
        ...
        if dist < dist_center - self.radius:
            return False
        for v in self.vertices:
            src_vertex_distance = src.dist(v)
            if src_vertex_distance < dist:
                dst_vertex_distance = dst.dist(v)
                if dist == dst_vertex_distance + src_vertex_distance:
                    return True
