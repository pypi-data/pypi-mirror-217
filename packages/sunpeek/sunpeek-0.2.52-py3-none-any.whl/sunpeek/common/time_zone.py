...
def process_tz_string(tz_string: Union[str, pytz.BaseTzInfo, pytz._FixedOffset, None], plant: 'sunpeek.components.Plant' = None):
    if tz_string is None:
        return None
    if isinstance(tz_string, pytz.BaseTzInfo):
        return tz_string
    if isinstance(tz_string, pytz._FixedOffset):
        return tz_string

    try:
        return pytz.timezone(tz_string)
    except pytz.exceptions.UnknownTimeZoneError:
        if tz_string == TIMEZONE_NONE:
            return None
        if tz_string == TIMEZONE_LOCAL_NO_DST:
            if plant is None:
        ...
        if tz_string == TIMEZONE_LOCAL_WITH_DST:
            if plant is None:
                raise TimeZoneError(f'Time zone "{tz_string}" requires a plant but got a None plant.')
