...
class TimeSeriesStoreAsyncTest:
    ...
    async def _bulk_read_mixed(self, start: datetime, end: datetime) -> Iterable[pd.DataFrame]:
        async with self.STORE.client as client:
    ...
    async def test_04_duplicated_read_bulk_mixed(self) -> None:
        async with self.STORE.client as client:
    ...
    async def test_09_async_iters(self) -> None:
        async with self.STORE.client as client:
    ...
    async def test_10_delete_data(self) -> None:
        async with self.STORE.client as client:
            start = datetime(year=2020, month=1, day=2, tzinfo=timezone.utc)
    ...
    async def test_11_delete_time_series(self) -> None:
        async with self.STORE.client as client:
