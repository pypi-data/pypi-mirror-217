...
class MemoryTimeSeriesClient(TimeSeriesClient):
    ...
    async def create_time_series(
        self, *, path: str, metadata: dict[str, Any] | TimeSeriesMetadata | None = None, **kwargs: Any
    ) -> TimeSeries:
    ...
    async def read_ensemble_members(
        ...
        **kwargs: Any,
        ...
        for ensemble in ts_data:
            if (t0_start is None or ensemble.t0 is not None and t0_start <= ensemble.t0) and (
                t0_end is None or ensemble.t0 is not None and ensemble.t0 <= t0_end
            ):
                yield ensemble
