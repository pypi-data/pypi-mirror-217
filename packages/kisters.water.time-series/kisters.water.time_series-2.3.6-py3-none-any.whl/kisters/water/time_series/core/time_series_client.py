...
class TimeSeriesClient:
    ...
    def run_sync(self, coroutine: Callable[..., Coroutine[Any, Any, T]], *args: Any, **kwargs: Any) -> T:
        ...
        1. Its a fully sync context, it's the main thread and no event loop is
           running.
        ...
        3. There is an event loop running in our thread, but this is a sync
           function damn it so we can't await.
        ...
        2. Using asyncio.get_event_loop() will create a event loop, but only if
           its the main thread. It will also leave the loop running, even when
           its not needed anymore. This also modifies global state and can break
           the assumptions of other packages. Moreover, it is known to have
           complex behaviour, so its best to avoid this method when possible.
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(asyncio.run, dispatched_execution())
    ...
    async def read_ensemble_members_bulk(
        ...
        **kwargs: Any,
        ...
        if default_factory:
            bulk_kwargs["default_factory"] = default_factory
        results: list[list[EnsembleMember]] = [[]] * len(paths)
        ensemble_a_iters = [
    ...
    async def read_comments_bulk(
        ...
        **kwargs: Any,
        ...
        if default_factory:
            bulk_kwargs["default_factory"] = default_factory
        results: list[list[TimeSeriesComment]] = [[]] * len(paths)
        comments_a_iters = [
