...
class BaseLabelme(metaclass=SingletonMeta):
    ...
    def model_to_iou(self, model_dataset, parameter):
        ...
        images_precision = round(right_check / model_positive, 4)
        # 图像召回率=right_check/mark_positive
        images_recall = round(right_check / mark_positive, 4)
        # 图像准确率=(正确 + 背景) / (正确 + 背景 + 误检 + 漏检)，
        images_accuracy = round((right_check + negative_sample) / (right_check + negative_sample + error_check + leak_check))
