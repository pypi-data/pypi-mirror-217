...
class TestCompletionsAddPerforation:
    ...
    def test_completions_set_add_perforation_bad_input(self, completions_set, unsupported_version_2019):
        ...
        # Slightly different check for this test as response contains 'long' locally, but not in pipeline
        assert "'Hello' has type str, but expected one of: int, " in error.value.args[0]
        assert "float" in error.value.args[0]

    def test_completions_set_add_perforation(self, completions_set, delete_workflow, unsupported_version_2019):
        if unsupported_version_2019 == True:
            pytest.skip("Unsupported Petrel version: 2019")
        try:
