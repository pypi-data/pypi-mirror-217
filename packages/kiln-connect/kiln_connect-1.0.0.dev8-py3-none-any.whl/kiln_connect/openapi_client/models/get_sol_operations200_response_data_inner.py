...
class GetSolOperations200ResponseDataInner(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetSolOperations200ResponseDataInner.construct()
        error_messages = []
        # validate data type: SOLOperationCreateAccountWithSeed
        if not isinstance(v, SOLOperationCreateAccountWithSeed):
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetSolOperations200ResponseDataInner with anyOf schemas: SOLOperationCreateAccount, SOLOperationCreateAccountWithSeed, SOLOperationDeactivate, SOLOperationDelegate, SOLOperationMerge, SOLOperationRedelegate, SOLOperationSplit, SOLOperationWithdraw. Details: " + ", ".join(error_messages))
        else:
            return v
