...
class GetEthOperations200ResponseDataInner(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetEthOperations200ResponseDataInner.construct()
        error_messages = []
        # validate data type: ETHOperationDeposit
        if not isinstance(v, ETHOperationDeposit):
        ...
        if not isinstance(v, ETHOperationConsensusWithdrawal):
        ...
        if not isinstance(v, ETHOperationExecutionReward):
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetEthOperations200ResponseDataInner with anyOf schemas: ETHOperationConsensusWithdrawal, ETHOperationDeposit, ETHOperationExecutionReward. Details: " + ", ".join(error_messages))
        else:
            return v
