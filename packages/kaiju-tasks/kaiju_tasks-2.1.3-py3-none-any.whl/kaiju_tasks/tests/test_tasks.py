...
class TestTaskExecutor:
    ...
    async def test_executor_suspend(self, _service: TaskExecutor, mock_stream_client):
        ...
        reqs = [stream.get_nowait()[0]['method'] for _ in range(stream.qsize())]
    ...
    async def test_task_execution(self, _service: TaskExecutor, mock_stream_client):
        ...
        methods = [r['method'] for r in reqs if r['method'] != 'taskman.ping']
        params = [r['params'] for r in reqs if r['method'] != 'taskman.ping']
        assert methods == [
