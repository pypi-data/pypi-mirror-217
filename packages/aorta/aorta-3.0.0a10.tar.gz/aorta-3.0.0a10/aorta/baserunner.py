...
class BaseRunner:
    ...
    async def run(self, envelope: Envelope[Any]) -> bool:
        ...
        # Commands are assumed to have one and only one handler,
        # so if it fails then there is no need to figure out
        # which handlers should be re-ran. In this case, the
        # messaging infrastructure must determine when to re-send
        # the message.
        if envelope.is_command() and retry:
            return False
