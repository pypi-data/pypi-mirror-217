...
class RunScriptManager:
    def __init__(self,databaseType,DATABASE_NAME,PROJECT_NAME,SERVER_NAME_AND_INSTANCE=None,TNS_ENTRY=None,slackEndpoint=None):
    ...
    def runScript(self,command):
        ...
        This function is run in the runScriptStep function.
            Args: 
                This function takes the command to be run on the command line as the argument. 
            Returns: 
        ...
        if self.databaseType=="MSSQL": 
            ...
            print(f"ERROR: {self.databaseType} is an nvalid database type. Please use either 'MSSQL' or 'Oracle'.")   
            logging.error(f"ERROR: {self.databaseType} is an nvalid database type. Please use either 'MSSQL' or 'Oracle'.")
            self.send_slack_message(f"ERROR: {self.databaseType} is an nvalid database type. Please use either 'MSSQL' or 'Oracle'.")
    ...
    def runScriptStep(self,step,scriptName,scriptPath):
        try: 
            ...
            if self.databaseType == 'MSSQL':
                if self.runScript(f"sqlcmd -S {self.SERVER_NAME_AND_INSTANCE} -i {scriptPath} -d {self.DATABASE_NAME} -E","MSSQL"):
                    self.send_slack_message(f"{self.PROJECT_NAME} Step {step} is successful!",self.slackEndpoint)
                else: 
                    self.send_slack_message(f"ERROR: {self.PROJECT_NAME} Step {step} has failed. {scriptName} has failed.",self.slackEndpoint)
                    sys.exit()
            if self.databaseType == "Oracle":
                if self.runScript(f"sqlplus {self.TNS_ENTRY} @{scriptPath}","Oracle"):
                    self.send_slack_message(f"{self.PROJECT_NAME} Step {step} is successful!",self.slackEndpoint)
                else: 
                    self.send_slack_message(f"ERROR: {self.PROJECT_NAME} Step {step} has failed. {scriptName} has failed.",self.slackEndpoint)
                    sys.exit()
        except Exception as e: 
            self.send_slack_message(f"ERROR: {self.PROJECT_NAME} Step {step} has failed. An Error occurred: {e}.",self.slackEndpoint)
            sys.exit()

class VirtualisationEngineSessionManager:
    def __init__(self, address, username, password, major, minor, micro, PROJECT_NAME, CONTAINER_NAME, slackEndpoint=None):
        now = datetime.datetime.now()
        formatted_date_time = now.strftime("%Y-%m-%d")
        logging.basicConfig(filename=f'DelphixEngineSessionManager_{formatted_date_time}.log',
                            level=logging.INFO, format='%(asctime)s %(levelname)s:%(message)s', filemode='a')
...
class MaskingEngineSessionManager(): 
    def __init__(self, address, username, password, PROJECT_NAME, ENVIRONMENT_NAME, slackEndpoint=None): 
        now = datetime.datetime.now()
        formatted_date_time = now.strftime("%Y-%m-%d")
        logging.basicConfig(filename=f'DelphixEngineSessionManager_{formatted_date_time}.log',
                            level=logging.INFO, format='%(asctime)s %(levelname)s:%(message)s', filemode='a')
