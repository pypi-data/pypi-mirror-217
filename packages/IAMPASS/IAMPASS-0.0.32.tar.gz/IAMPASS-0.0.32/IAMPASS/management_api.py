...
class ManagementAPI:
    """
        Wrapper class for the IAMPASS management REST API.
    ...
    def add_users(self, users: [str]):
        """
            Registers users with IAMPASS.

            Calls the IAMPASS service to register users.
        ...
        url = "{}/management/add_users/{}".format(IAMPASS_SERVER, self.application_id)
    ...
    def add_user(self, user: str):
        """
            Registers a single with IAMPASS.

            Calls the IAMPASS service to register a single user
    ...
    def delete_users(self, user_ids: [str]):
        """
            Deletes users from IAMPASS.

            Calls the IAMPASS service to delete users
        ...
        url = "{}/management/delete_users/{}".format(IAMPASS_SERVER, self.application_id)
    ...
    def delete_user(self, user_id: str):
        """
            Deletes a single from IAMPASS.

            Calls the IAMPASS service to delete a single user
    ...
    def get_registration_link(self, user_id: str, display_name: str):
        """
            ...
            Calls the IAMPASS service to obtain a device registration for a user.
            The registration link is intended to be called from a mobile application to register a particular device.

            Parameters:
            user_id (str): The user ID.
            display_name (str): IAMPASS does not store personal user information. The display_name parameter
                                allows IAMPASS to generate a device registration link that includes a displayable
                                name for the user.
            .
        ...
        url = "{}/management/device_registration_link/{}/{}{}".format(IAMPASS_SERVER,
                                                                      self.application_id,
                                                                      encoded_user,
                                                                      display_name_query)
    ...
    def send_registration_sms(self, user_id: str, display_name: str, phone_number: str, message):
        ...
        url = "{}/management/send_registration_sms/{}/{}".format(IAMPASS_SERVER,
                                                                 self.application_id,
                                                                 encoded_user)
    ...
    def has_registered_mobile_device(self, user_id: str):
        ...
        url = "{}/management/has_registered_mobile_device/{}/{}".format(IAMPASS_SERVER,
                                                                        self.application_id,
                                                                        encoded_user)
