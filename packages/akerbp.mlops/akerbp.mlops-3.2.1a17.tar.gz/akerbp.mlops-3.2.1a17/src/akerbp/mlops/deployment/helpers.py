...
def get_deployment_folder_content(
    ...
    def walk_folder(path: Path):
        ...
        for entry in path.iterdir():
            if str(venv_dir) in entry.name.split("/"):
                continue
...
def do_deploy(
    ...
    **kwargs,
    ...
    if setup_venv:
        ...
        # Deal with content of virtual env
        deployment_folder_content_keys = list(deployment_folder_content.keys())
        for key in deployment_folder_content_keys:
            if str(venv_dir) in key:
                del deployment_folder_content[key]
    else:
