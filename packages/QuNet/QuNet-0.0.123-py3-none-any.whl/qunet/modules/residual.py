...
class ResidualAlign(nn.Sequential):
    ...
    """
    def __init__(self, Ein, Eout, norm, dim):            

        if dim==1: layers = [ nn.Linear(Ein, Eout) ]
        else:      layers = [ nn.Conv2d(Ein, Eout, kernel_size=1, stride=2, bias=False) ]

        if norm == 1:
            layers.append( nn.BatchNorm2d(Eout)    if dim==2 else nn.BatchNorm1d(Eout) )
        elif norm == 2:
            layers.append( nn.InstanceNorm2d(Eout) if dim==2 else nn.LayerNorm (Eout) )
        print(layers)
...
class ResidualAfter(nn.Sequential):
    ...
    def __init__(self, E, norm, dim, fun):                    
        layers = []
        if norm == 1:
            layers.append( nn.BatchNorm2d(E)    if dim==2 else nn.BatchNorm1d(E) )
        if norm == 2:
            layers.append( nn.InstanceNorm2d(E) if dim==2 else nn.LayerNorm (E) )
        if fun:
...
class Residual(nn.Module):
    ...
    ```
    """
    def __init__(self, block, E, Eout=None, res = 1, gamma=0.1, drop=0., dim=1, 
        ...
        if Eout == E:
            ...
            self.align = ResidualAlign(E, Eout, norm_align, dim)
