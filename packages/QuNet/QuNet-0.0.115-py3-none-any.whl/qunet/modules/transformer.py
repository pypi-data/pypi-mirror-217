...
class Residual(nn.Module):
    ...
    """
    def __init__(self, module, E, res = 1, skip=1., gamma=0.1, drop=0., vit2d=False):
        super().__init__()
...
class  TransformerBlock(nn.Module):
    ...
    def create(self):
        ...
        if cfg.is_fft:
            self.fft = Residual(FFT(cfg.fft),       E=cfg.att.E, res=cfg.fft.res, skip=cfg.fft.skip, gamma=cfg.gamma, drop=cfg.drop)
        if cfg.is_att:
            self.att = Residual(Attention(cfg.att), E=cfg.att.E, res=cfg.att.res, skip=cfg.att.skip, gamma=cfg.gamma, drop=cfg.drop)
        if cfg.is_mlp:
            self.mlp = Residual(MLP(cfg.mlp),       E=cfg.att.E, res=cfg.mlp.res, skip=cfg.mlp.skip, gamma=cfg.gamma, drop=cfg.drop)
    ...
    def set_drop(self, fft, att, mlp):
        if self.cfg.is_fft:
            self.fft.set_drop( fft )
            #self.fft.module.set_drop( fft )

        if self.cfg.is_att:
            self.att.set_drop( att )
            self.att.module.set_drop( att )

        if self.cfg.is_mlp:
            self.mlp.set_drop( mlp )
