...
# dmFromSpinW = np.loadtxt(r'C:\Users\lass_j\Desktop\testing2.txt')

# dm1s = np.asarray([dm for dm in dmFromSpinW.T if np.isclose(np.linalg.norm(dm[-3:],axis=0),0.4322,atol=0.01)]).T
...
epsilon=0.35;
omega=np.arange(0,16.0,epsilon/3)#
...
QRLU = np.concatenate([QRLU1,QRLU2],axis=1)
...
for coup in S.lattice.couplings:
    ...
    # newCoupling = Coupling(coup.atom1, coup.atom2, coup.atom1Pos, coup.atom2Pos, coup.dl, 
                           # coup.distanceVector, coup.distance,label=title,
                           # exchange=exchange)
    
    # S.lattice.couplings.append(newCoupling)
    
#     print(c.atom1,c.atom2,c.dl,dlTest,rotAngle)
    # h=kk
