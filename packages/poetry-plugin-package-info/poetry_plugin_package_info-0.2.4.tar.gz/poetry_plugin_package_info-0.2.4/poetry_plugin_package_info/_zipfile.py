...
class ZipInfo:
    ...
    def __init__(
        ...
        self.create_version = (
            DEFAULT_VERSION  # Version which created ZIP archive
        )
        self.extract_version = (
            DEFAULT_VERSION  # Version needed to extract archive
        )
...
class _SharedFile:
    def __init__(self, file, pos, close, lock, writing) -> None:
        self._file = file
        self._pos = pos
        self._close = close
        self._lock = lock
...
class ZipExtFile(io.BufferedIOBase):
    ...
    def __init__(
        ...
        if pwd:
            ...
            if h != check_byte:
                raise RuntimeError(
                    "Bad password for file %r" % zipinfo.orig_filename,
                )
...
class _ZipWriteFile(io.BufferedIOBase):
    def __init__(self, zf, zinfo, zip64) -> None:
        self._zinfo = zinfo
        self._zip64 = zip64
        self._zipfile = zf
...
class ZipFile:
    ...
    def _extract_member(self, member, targetpath, pwd):
        ...
        with self.open(member, pwd=pwd) as source, open(
            targetpath,
            "wb",
        ) as target:
    ...
    def close(self):
        ...
        try:
            if (
                self.mode in ("w", "x", "a") and self._didModify
...
def main(args=None):
    ...
    if args.test is not None:
        ...
        if badfile:
            print(
                "The following enclosed file is corrupted: {!r}".format(
                    badfile,
                ),
            )
        print("Done testing")
