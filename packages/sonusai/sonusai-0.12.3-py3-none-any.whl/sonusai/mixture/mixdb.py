...
class MixtureDatabaseConfig(DataClassSonusAIMixin):
    asr_manifest: Optional[List[Location]] = field(default_factory=list)
...
class MixtureDatabase:
    ...
    def asr_manifest(self) -> List[Location]:
        return self.config.asr_manifest

    @asr_manifest.setter
    def asr_manifest(self, value: List[Location]) -> None:
        self.config.asr_manifest = value
    ...
    def target_asr_data(self, target: TargetFile) -> Union[List[str], List[None]]:
        ...
        name, _ = tokenized_expandvars(target.name)
        return self._asr_manifest_data.get(name, None)

    def mixid_asr_data(self, mixid: int) -> Union[List[str], List[None]]:
    ...
    def load_asr_data(self) -> None:
        ...
        def _error_preamble(name: Location, line_num: int) -> str:
            return f'Invalid entry in ASR manifest {name} line {line_num}'

        self._asr_manifest_data = {}

        for name in self.config.asr_manifest:
            with open(file=name, mode='r') as f:
                ...
                for line in f:
                    ...
                    key, _ = tokenized_expandvars(result['audio_filepath'])
                    value = result['text']
