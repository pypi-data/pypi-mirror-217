...
def phase_distance(reference: np.ndarray, hypothesis: np.ndarray) -> (np.ndarray, np.ndarray, np.ndarray):
...
def _process_mixture(mixid: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    segsnr_t = mixdb.mixture_segsnr_t(mixid, target=target, noise=noise)
    segsnr_f = mixdb.mixture_segsnr(mixid, segsnr_t=segsnr_t, target=target, noise=noise)
    ...
    ssnravg = 10 * np.log10(np.mean(segsnr_f) + 1e-7)
    ssnrp75 = 10 * np.log10(np.percentile(segsnr_f, 75) + 1e-7)
    ssnrpk = 10 * np.log10(np.max(segsnr_f) + 1e-7)
    segsnr_fdb = 10 * np.log10(segsnr_f + 1e-7)
    ...
    if wer_method == 'none':
        ...
        asr_tt = calc_asr(target, engine=wer_method, whisper_model_name=whisper_model)  # target truth
        if asr_tt.text:
            asr_mx = calc_asr(mixture, engine=wer_method, whisper_model_name=whisper_model)
            asr_tge = calc_asr(target_est_wav, engine=wer_method, whisper_model_name=whisper_model)
    ...
    new_labels = pd.MultiIndex.from_product([metr2.columns,
                                             ['Avg', 'Min', 'Med', 'Max', 'Std']],
                                            names=['Metric', 'Stat'])
    dat1row = metr2.loc[['mean', 'min', '50%', 'max', 'std'], :].stack().to_numpy().reshape((1, -1))
    ...
    with open(metric_name, 'w') as f:
        ...
        if wer_method != 'none':
            ...
            # print(f'WER method: {wer_method} and truth:  {asr_tt}', file=f)
            # print(f'If Whisper used, model: {whisper_model}', file=f)
...
def main():
    ...
    mixdb = MixtureDatabase(config=truth_location, show_progress=True)
    mixids = mixdb.mixids_to_list(mixids)
    logger.info(
            f'Found mixdb of {len(mixdb.mixtures)} total mixtures, with {mixdb.num_classes} classes in {truth_location}')
