...
def genmixdb(location: Location,
    ...
    if logging:
        ...
        target_audio_samples = {f'{x.target_file_index}_{x.target_augmentation_index}': x.samples for x in
                                mixdb.mixtures}
        target_audio_samples = sum(target_audio_samples.values())
        raw_noise_audio_duration = sum([x.duration for x in mixdb.noises])
        ...
        total_noise_files = sum([len(elem) for elem in mixdb.augmented_noise_audios])
        augmented_noise_audio_samples = sum(
                [len(elem) for elem in [item for sl in mixdb.augmented_noise_audios for item in sl]])
        augmented_target_audio_samples = 0
        for augmented_target in augmented_targets:
            augmented_target_audio_samples += estimate_audio_length(
                    audio=mixdb.raw_target_audios[augmented_target.target_file_index],
