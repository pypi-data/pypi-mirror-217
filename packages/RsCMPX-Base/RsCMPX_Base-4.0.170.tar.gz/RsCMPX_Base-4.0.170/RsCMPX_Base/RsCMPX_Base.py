...
class RsCMPX_Base:
	...
	_driver_options = "SupportedInstrModels = CMX500/CMP/CMW/PVT, SupportedIdnPatterns = CMX/CMP/CMW/PVT, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,4.0.170.0021'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.170.0021'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.170.0021'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_Base version failed. Current version: '4.0.170.0021', minimum required version: '{min_version}'")
	...
	def set_macro_enable(self, enable: bool) -> None:
		...
		Enables or disables the execution of all macros that are defined for the active remote connection. Note: In contrast to
		SCPI specifications, macro execution is disabled by default. \n
			:param enable: Boolean value to enable or disable macro execution. In the disabled state (OFF / 0) , macros in a command sequence are not expanded. The R&S CMP200 issues an error message: 113, Undefined header;MacroLabel.
		"""
