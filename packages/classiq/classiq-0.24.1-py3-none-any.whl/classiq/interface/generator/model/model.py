...
from classiq.interface.generator.model.name_resolution import resolve_function_calls
...
from classiq import ForeignFunctionDefinition
...
class Model(VersionedModel):
    ...
    def update_function_calls(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        resolve_function_calls(
            values,
            nameables_to_dict(
                values.get("classical_functions")  # type:ignore[arg-type]
            ),
            nameables_to_dict(values.get("functions")),  # type:ignore[arg-type]
        )
        return values
