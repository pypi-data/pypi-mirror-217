...
def execute(qunatum_program: SerializedQuantumProgram) -> ResultsCollection:
    circuit = GeneratedCircuit.parse_raw(qunatum_program)
    return async_utils.run(ApiWrapper.call_execute_generated_circuit(circuit)).results
