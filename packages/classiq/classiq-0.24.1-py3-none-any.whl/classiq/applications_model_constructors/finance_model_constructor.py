...
from classiq.interface.generator.expressions.finance_functions import (
...
_OUTPUT_VARIABLE_NAME = "derivative_estimation"
...
def construct_finance_model(
    ...
    if isinstance(finance_function_input.f, FinanceFunction):
        ...
        finance_function_f = FinanceFunction(finance_function_input.f)
    ...
    finance_function = f"struct_literal(FinanceFunction, {{f: {finance_function_f.to_name()}, threshold: {finance_function_input.condition.threshold}, larger: {finance_function_input.condition.larger}, polynomial_degree: {polynomial_degree}, use_chebyshev_polynomial_approximation: {finance_function_input.use_chebyshev_polynomial_approximation}, tail_probability: {tail_probability}}})"

    model = Model(
        constraints={
            "max_width": phase_port_size + (finance_model_input.num_qubits + 1) * 2 + 5
        },
        functions=[
            *QMCI_LIBRARY,
            NativeFunctionDefinition(
                    ...
                    QuantumFunctionCall(
                        ...
                        params={
                            "num_unitary_qubits": Expression(
                                expr=f"{finance_model_input.num_qubits}+1"
                            ),
                ...
                body=[
                    ...
                    AssignmentStatement(
                        ...
                        invoked_expression=ClassicalFunctionCall(
                            ...
                            params={
                                "num_shots": Expression(expr=f"{num_shots}"),
                    ...
                    AssignmentStatement(
                        ...
                        invoked_expression=Expression(
                            expr=f"{post_process_function}({finance_model}, {finance_function}, estimation)",
                        ),
                    ),
