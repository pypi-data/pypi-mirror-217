...
from inhandtest.base_page import IgTable
...
class MeasureMonitor(BasePage, IgLocators):
    ...
    def get_status(self, measure_name: str or list or tuple) -> str or dict or None:
        """
        ...
        if measure_name:
            ...
            if isinstance(measure_name, str):
                return self.get_text('value', self.edge_locators.measure_card_status(measure_name))
            else:
                measure_value = {}
                for name in measure_name:
                    measure_value.update({name: self.get_text('value', self.edge_locators.measure_card_status(name))})
