...
class DbSync:
    ...
    def table_name(self, stream_name, is_temporary=False, without_schema=False):
        ...
        if is_temporary:
            mssql_table_name = f'{mssql_table_name.lower()}_temp'
    ...
    def bulk_insert_records(
        ...
        self.logger.info(f"Preparing records for bulk insertion into table {full_table_name}.")
        ...
        for record in records.values():
            ...
            for key in keys:
                if key in record:
                    ...
                    record_values.append('')  # Handle missing keys gracefully by inserting None
            values.append(tuple(record_values))
        self.logger.info(f"Records prepared, bulk inserting it into table {full_table_name}.")
        return values
    ...
    def process_batch(self, records) -> None:
        ...
        tmp_table_name_wo_schema = self.table_name(stream_schema_message['stream'], is_temporary=True, without_schema=True)
        ...
        isException = False
        # Insert into table
        conn = self.open_connection(auto_commit=False)
        if key_properties:
            ...
            try:
                conn.bulk_copy(table_name=tmp_table_name, elements=values)
                conn.commit()
            except Exception as e:
                self.logger.info(f"Exception111:::: {e}")
                isException = True
                conn.rollback()
            finally:
                conn.close()
        ...
        if key_properties:
            try:
                if isException:
                    conn.bulk_copy(tmp_table_name_wo_schema, values)
                    conn.commit()
            except Exception as e:
                self.logger.info(f"Exception2222:::: {e}")
                conn.rollback()
            finally:
                conn.close()
        else:
            ...
            conn.bulk_copy(full_table_name, values)
            conn.commit()
            conn.close()
