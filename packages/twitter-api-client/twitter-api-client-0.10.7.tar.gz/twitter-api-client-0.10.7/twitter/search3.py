...
class Search:
    ...
    async def paginate(self, client: AsyncClient, query: dict, limit: int, out: Path, **kwargs) -> list[dict]:
        ...
        while True:
            ...
            if self.debug:
                self.logger.debug(f'{c}{query}{reset} {len(total) = }')

            if len(total) >= limit:
                if self.debug:
                    self.logger.debug(
                        f'[{GREEN}success{RESET}] Returned {len(total)} search results for {c}{query}{reset}')
                return res

            if self.save:
                (out / f'{time.time_ns()}.json').write_text(
                    orjson.dumps(entries, option=orjson.OPT_INDENT_2).decode(),
                    encoding='utf-8'
                )
