...
from .configuration import OutboundSyncConfigurationParameters,InboundSyncConfigurationParameters, OutboundSyncStrategy, StoredConfigurationValue, StoredMappingValue,SyncConfigurationParameters,StreamConfiguration, StoredStreamConfiguration
...
class PluginEntrypoint():
    ...
    def sync(self,sync_request:Dict):
        ...
        if request.sync_direction=='outbound':
            ...
            inbound_sync_request._thread_cancellation_token.set() # cancel the thread so we don't leave anything hanging around and cop a nasty error
            inbound_sync_request._apply_results_task.join()
            inbound_sync_request._cancel_checking_task.join()
            return_dict["streams"] = [s.dict() for s in streams_list]
            return_dict["errored_streams"] = list(omnata_log_handler.stream_has_errors.keys())
    ...
    def outbound_record_validator(self,sync_parameters:Dict,field_mappings:Dict,transformed_record:Dict,source_types:Dict[str,str]):
        ...
        sync_parameters:Dict[str, StoredConfigurationValue] = parse_obj_as(Dict[str, StoredConfigurationValue],sync_parameters)
        field_mappings:StoredMappingValue = parse_obj_as(StoredMappingValue,field_mappings)
        return self._plugin_instance.outbound_record_validator(sync_parameters,field_mappings,transformed_record,source_types)
