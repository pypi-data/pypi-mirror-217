...
def listMissingPackagesFromImports(importedModules: Optional[List[str]],
  ...
  for im in importedModules:
    ...
    assert baseModuleInst is not None
    if isStdlibModule(baseModuleInst):
      continue  # module is a system module, e.g. json
    if baseModule not in installedModules:
      continue  # probably from a local file, not an installed package
...
def listLocalModulesFromImports(importedModules: Optional[List[str]]) -> List[str]:
  ...
  for im in importedModules:
    ...
    if baseModule not in installedModules:
      baseModuleInst = sys.modules.get(baseModule)
      assert baseModuleInst is not None
      if not isStdlibModule(baseModuleInst):
        localModules.append(baseModule)
  return localModules


def isStdlibModule(mod: ModuleType) -> bool:
  if mod.__name__ in sys.builtin_module_names:
    return True
  if not hasattr(mod, "__file__") or mod.__file__ is None:
    return True
  return mod.__file__.startswith((sys.prefix, sys.base_prefix))
