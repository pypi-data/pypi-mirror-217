...
from owe_python_sdk.utils import adapt_runner
from owe_python_sdk.errors import RunnerAdaptationError


class Runnable:
    def _set_runner_class(self, RunnerClass: Runner=None):
        # Try to adapt the runner class if provided instance of runner
        # does not inherit from Runner 
        try:
            if RunnerClass != None and not issubclass(RunnerClass, Runner):
                RunnerClass = adapt_runner(Runner, RunnerClass)
        ...
        self.runner_class = RunnerClass

class Task(Runnable):
    def __init__(self, schema: dict, runner: Runner=None):
        self._set_runner_class(runner)
...
class Pipeline(Runnable):
    def __init__(self, schema: dict, runner: Runner):
    ...
    def start(self, params: owe_schema.KeyVal={}):
