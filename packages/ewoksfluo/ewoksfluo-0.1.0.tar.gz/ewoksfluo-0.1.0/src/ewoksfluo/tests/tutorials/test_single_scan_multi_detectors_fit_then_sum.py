...
def test_single_scan_multi_detectors_fit_then_sum_without_qt(tmpdir):
    from orangecontrib.ewoksfluo import tutorials

    with resources.path(
        tutorials, "single_scan_multi_detectors_fit_then_sum.ows"
    ) as filename:
        assert_single_scan_multi_detectors_fit_then_sum_without_qt(filename, tmpdir)


def test_single_scan_multi_detectors_fit_then_sum_with_qt(ewoks_orange_canvas, tmpdir):
    from orangecontrib.ewoksfluo import tutorials

    with resources.path(
        tutorials, "single_scan_multi_detectors_fit_then_sum.ows"
    ) as filename:
        assert_single_scan_multi_detectors_fit_then_sum_with_qt(
            ewoks_orange_canvas, filename, tmpdir
        )


def assert_single_scan_multi_detectors_fit_then_sum_without_qt(filename, tmpdir):
...
def assert_single_scan_multi_detectors_fit_then_sum_with_qt(
    ewoks_orange_canvas, filename, tmpdir
):
...
def get_inputs(tmpdir) -> List[dict]:
    return [
        ...
        {
            "label": "Multi-Detector Fit",
            "name": "output_uri_template",
            "value": str(tmpdir / "output.h5::/1.1/fit_{detector_name}"),
        },
        {
            "label": "Sum Fit Results",
            "name": "output_uri",
            "value": str(tmpdir / "output.h5::/1.1/sum_of_fit"),
        },
        {
            "label": "Sum Fit Results",
            "name": "scan_uri",
            "value": str(tmpdir / "input.h5::/1.1"),
        },
        {
            "label": "Normalization",
            "name": "output_uri",
...
def get_expected_outputs(tmpdir) -> Dict[str, dict]:
    return {
        ...
        "Multi-Detector Fit": {
            "fit_results_uri_template": str(
                tmpdir / "output.h5::/1.1/fit_{detector_name}/results"
            ),
            "detector_names": ["mca0", "mca1"],
        },
        "Sum Fit Results": {
            "summed_results_uri": str(tmpdir / "output.h5::/1.1/sum_of_fit/results")
        },
        "Normalization": {
