...
Integral = int
Number = Union[float, Integral]


class EmissionLineGroup(NamedTuple):
    element: str
    name: str
    counts: Union[Integral, Sequence]
...
def get_counts_array(counts: Union[Integral, Sequence]):
...
def xrf_spectra(
    ...
    nchannels: Integral = 1024,
    energy: Number = 7.5,
    angle_in: Number = 62.0,
    angle_out: Number = 49.0,
    flux: Number = 1e10,
    elapsed_time: Number = 1.0,
...
def get_quantification_config(flux: Number = 1e10, elapsed_time: Number = 1.0) -> dict:
    return {
        "usematrix": 0,
        "useattenuators": 1,
...
def get_fit_config(xmin: Integral = 0, xmax: Integral = 0, energy: Number = 0) -> dict:
    return {
        "deltaonepeak": 0.01,
...
def get_attenuators(angle_in: Number = 62.0, angle_out: Number = 49.0) -> dict:
    return {
...
def get_detector(zero: Number = 0, gain: Number = 0.005) -> dict:
    return {
        "detele": "Si",
        "nthreshold": 4,
        "zero": zero,
