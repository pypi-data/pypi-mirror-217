...
nqx=100
nqy=100
qpt_cart = gen_uniform_qmesh(nqx,nqy,bound=8)
...
if __name__=='__main__':
    for conf_name in all_confs:
        ...
        if conf_name in ['Tetrahedra','Cubic']:  spin_plot_kws.update(colorful_quiver=True)
        ...
        ax_plot_spin_2d(ax['a'],sites_cart,sp_lat,**spin_plot_kws)
        subplot_struct_factor(ax['b'],qpt_cart,S_vector,scatter_size=2,nqx=nqx,nqy=nqy,comp='parall',nn=2)
        subplot_struct_factor(ax['c'],qpt_cart,S_vector,scatter_size=2,nqx=nqx,nqy=nqy,comp='normal',nn=2)
