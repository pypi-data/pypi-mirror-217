...
class SQLiteDataStorage(AbstractDataStorage):
    ...
    def create_signals_table(
        self, uri: str, cols: List[Tuple[str, "TypeEngine"]]
    ) -> SignalsTable:
        """
        Create a signals table for storing signals entries.
        """
        tbl_name = self._table_name(self.uri, self.SIGNALS_TABLE_PREFIX)
        columns = [saColumn(col_name, col_type) for (col_name, col_type) in cols]
        tbl = SignalsTable(tbl_name, columns, self.metadata)
