...
from omnata_plugin_runtime.omnata_plugin import InboundSyncRequest, OmnataPlugin, PluginManifest, ConnectResponse, OutboundSyncRequest,managed_outbound_processing
from omnata_plugin_runtime.configuration import CreateSyncStrategy,UpdateSyncStrategy,OutboundSyncConfigurationParameters,ConnectionConfigurationParameters,InboundSyncConfigurationParameters,OutboundSyncStrategy,OutboundSyncAction
...
class TemplateApp(OmnataPlugin):
    ...
    def connection_form(self) -> List[ConnectionMethod]:
        ...
        """
        return [
            ConnectionMethod(name='API Key',fields=[
                FormInputField(name='api_key',label='API Key',required=True,secret=True)
            ])
        ]
    ...
    def connect(self, parameters: ConnectionConfigurationParameters) -> ConnectResponse:
        ...
        response = requests.get(url='https://login.template-app.com',
    ...
    def outbound_configuration_form(self,parameters: OutboundSyncConfigurationParameters) -> OutboundSyncConfigurationForm:
        return OutboundSyncConfigurationForm(
            fields=[
                FormDropdownField(name='target_object',label='Target Object',help_text='The object to sync data into',
            ...
            ],mapper=FormFieldMappingSelector(depends_on='target_object',
                    # this will be populated dynamically after the target object is selected
                    data_source=DynamicFormOptionsDataSource(source_function=self.field_mappings)
            )
        )
    ...
    def field_mappings(self,parameters: OutboundSyncConfigurationParameters):
        ...
        fields_response = requests.get(url=f'{tenant_url}/api/v1/fields/{target_object}', timeout=60)
        ...
        for field in fields['fields']:
            ...
            fields_to_return.append(FormOption(
                    value=field["name"],
                    label=field["label"],
    ...
    def sync_outbound(self,parameters: OutboundSyncConfigurationParameters,outbound_sync_request:OutboundSyncRequest):
        ...
        if parameters.sync_strategy.name==CreateSyncStrategy.name:
            ...
            for record in outbound_sync_request.get_records():
                # create the record
                response = requests.post(url=f'{tenant_url}/api/v1/{target_object}', json=record, timeout=60)
                if response.status_code != 200:
        ...
        self.create_records(outbound_sync_request.get_records(batched=True,sync_actions=['Create']),
    ...
    def create_records(self,data_frame:pandas.DataFrame,target_object:str,tenant_url:str) -> pandas.DataFrame:
        ...
        response = requests.post(url=f'{tenant_url}/api/v1/{target_object}/create', json=records_array, timeout=60)
        if response.status_code != 200:
        ...
        data_frame = pandas.merge(data_frame,results_frame,left_index=True,right_index=True)
        ...
        data_frame = data_frame.assign(SUCCESS=lambda x:'error' not in x or x['error']=='')
