...
class PluginUploader:
  ...
  ;
    ...
    def upload_plugin(self,plugin_directory:Path,
        ...
        subfolders = [ f.path for f in os.scandir('.packages') if f.is_dir() and '.packages/omnata_plugin_runtime-' in f.path ]
        if len(subfolders)!=1:
            raise ValueError("Failed to find omnata-plugin-runtime in downloaded package metadata")
        subfolder = subfolders[0]
        regex_matches = re.search(r"^.packages/omnata_plugin_runtime-(.*)\.dist-info$",subfolder)
        if regex_matches is None:
            raise ValueError("Failed to parse version of omnata-plugin-runtime package from metadata")
        ...
        has_custom_validator = plugin_class_instance.outbound_record_validator is not OmnataPlugin.outbound_record_validator
        ...
        with open(install_script,'w',encoding='utf-8') as setup_script:
            ...
            for proc_template in ['API_LIMITS','SYNC','CONFIGURATION_FORM','CONNECTION_TEST','CONNECTION_FORM','CREATE_BILLING_EVENTS','NETWORK_ADDRESSES','RETRIEVE_SECRETS','OUTBOUND_RECORD_VALIDATOR']:
