...
def get_user_information(url, token, **kwargs) -> dict:
    headers = {**_create_authorization_header(token), **kwargs.pop("headers", dict())}
    r = requests.get(url + "/api/users/by-token", headers=headers, **kwargs)
    r.raise_for_status()
    return r
...
class API:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.user = get_user(self.url, self.token, **kwargs)
        self.headers = _create_authorization_header(self.token)
