...
from PJYoloVision.yolo.utils import LOGGER, TESTS_RUNNING
...
def on_pretrain_routine_start(trainer, *args, **kwargs):
    try:
        experiment = comet_ml.Experiment(project_name=trainer.args.project or 'PJYoloVision')
...
def on_train_end(trainer, *args, **kwargs):
    experiment = comet_ml.get_global_experiment()
    if experiment:
        experiment.log_model('PJYoloVision', file_or_folder=str(trainer.best), file_name='best.pt', overwrite=True)
