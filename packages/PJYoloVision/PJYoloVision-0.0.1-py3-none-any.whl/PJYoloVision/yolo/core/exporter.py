...
from PJYoloVision.nn.modules import C2f, Detect, Segment
from PJYoloVision.nn.tasks import DetectionModel, SegmentationModel
from PJYoloVision.yolo.cfg import get_cfg
from PJYoloVision.yolo.utils import (DEFAULT_CFG, LINUX, LOGGER, MACOS, __version__, callbacks, colorstr,
                                     get_default_args, yaml_save)
from PJYoloVision.yolo.utils.checks import check_imgsz, check_requirements, check_version
from PJYoloVision.yolo.utils.files import file_size
from PJYoloVision.yolo.utils.ops import Profile
from PJYoloVision.yolo.utils.torch_utils import get_latest_opset, select_device, smart_inference_mode
...
def try_export(inner_func, *args, **kwargs):
    # PJYoloVision export decorator, i..e @try_export
...
class Exporter:
    ...
    def __call__(self, model=None, *args, **kwargs):
        ...
        description = f'PJYoloVision {self.pretty_name} model ' + f'trained on {Path(self.args.data).name}' \
    ...
    def _export_torchscript(self, prefix=colorstr('TorchScript:')):
        # PJYoloVision TorchScript model export
    ...
    def _export_coreml(self, prefix=colorstr('CoreML:')):
        # PJYoloVision CoreML export
        check_requirements('coremltools>=6.0')
        import coremltools as ct  # noqa

        class iOSDetectModel(torch.nn.Module ):
            # Wrap an PJYoloVision YOLO model for iOS export
    ...
    def _export_engine(self, workspace=4, detail=False, prefix=colorstr('TensorRT:')):
        # PJYoloVision TensorRT export https://developer.nvidia.com/tensorrt
    ...
    def _export_saved_model(self, prefix=colorstr('TensorFlow SavedModel:')):
        ...
        # PJYoloVision TensorFlow SavedModel export
        try:
    ...
    def _export_pb(self, keras_model, prefix=colorstr('TensorFlow GraphDef:')):
        # PJYoloVision TensorFlow GraphDef *.pb export https://github.com/leimao/Frozen_Graph_TensorFlow
        import tensorflow as tf  # noqa
    ...
    def _export_tflite(self, keras_model, nms, agnostic_nms, prefix=colorstr('TensorFlow Lite:')):
        # PJYoloVision TensorFlow Lite export
        import tensorflow as tf  # noqa
    ...
    def _export_tfjs(self, prefix=colorstr('TensorFlow.js:')):
        # PJYoloVision TensorFlow.js export
